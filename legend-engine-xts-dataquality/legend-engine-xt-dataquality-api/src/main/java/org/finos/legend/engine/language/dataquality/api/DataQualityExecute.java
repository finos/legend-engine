...
public class DataQualityExecute
{
    ...
    public List<RelationValidation> ruleSuggestions(@Context HttpServletRequest request, DataQualityProfileInput dataQualityProfilingInput, @DefaultValue(SerializationFormat.defaultFormatString) @QueryParam("serializationFormat") SerializationFormat format, @DefaultValue("true") @QueryParam("excludePlatformColumns") boolean excludePlatformColumns, @ApiParam(hidden = true) @Pac4JProfileManager() ProfileManager<CommonProfile> pm)
    {
        ...
        Identity identity = Identity.makeIdentity(profiles);
        long start = System.currentTimeMillis();
        LOGGER.info(new LogInfo(identity.getName(), DataQualityLoggingEventType.DATAQUALITY_RULE_SUGGESTION_START).toString());
        ...
        LOGGER.info(new LogInfo(identity.getName(), DataQualityLoggingEventType.DATAQUALITY_RULE_SUGGESTION_END, System.currentTimeMillis() - start).toString());
    ...
    private static Root_meta_external_dataquality_datarecon_DataQualityReconInput createReconInput(PureModel pureModel, Root_meta_external_dataquality_DataQualityRelationComparison dqComparisonElement, DataQualityReconInput input, org.finos.legend.pure.m3.coreinstance.meta.pure.metamodel.function.LambdaFunction<?> sourceLambdaFunction, org.finos.legend.pure.m3.coreinstance.meta.pure.metamodel.function.LambdaFunction<?> targetLambdaFunction)
    {
        ...
        {
            return core_dataquality_generation_datarecon.Root_meta_external_dataquality_datarecon_createReconInput_LambdaFunction_1__LambdaFunction_1__String_MANY__Boolean_1__String_MANY__String_$0_1$__String_$0_1$__Boolean_1__Integer_$0_1$__Boolean_1__Boolean_1__String_MANY__DataQualityReconInput_1_(
                    sourceLambdaFunction, targetLambdaFunction, input.keys == null ? Lists.immutable.empty() : Lists.immutable.ofAll(input.keys), input.aggregatedHash, input.colsForHash == null ? Lists.immutable.empty() : Lists.immutable.ofAll(input.colsForHash), input.sourceHashCol, input.targetHashCol, input.includeColumnValues, input.defectLimit, false, input.enrichDQColumns, input.additionalColumnsToPersist == null ? Lists.immutable.empty() : Lists.immutable.ofAll(input.additionalColumnsToPersist), pureModel.getExecutionSupport()
            );
        }
        return core_dataquality_generation_datarecon.Root_meta_external_dataquality_datarecon_createReconInput_LambdaFunction_1__LambdaFunction_1__String_MANY__Boolean_1__String_MANY__String_$0_1$__String_$0_1$__Boolean_1__Integer_$0_1$__Boolean_1__Boolean_1__String_MANY__DataQualityReconInput_1_(
                dqComparisonElement._source(), dqComparisonElement._target(), dqComparisonElement._keys(), getAggregatedHash(dqComparisonElement._strategy()), dqComparisonElement._columnsToCompare(), getSourceHashColumn(dqComparisonElement._strategy()), getTargetHashColumn(dqComparisonElement._strategy()), input.includeColumnValues, input.defectLimit, false, input.enrichDQColumns, dqComparisonElement._additionalColumnsToPersist(), pureModel.getExecutionSupport()
        );
    }
