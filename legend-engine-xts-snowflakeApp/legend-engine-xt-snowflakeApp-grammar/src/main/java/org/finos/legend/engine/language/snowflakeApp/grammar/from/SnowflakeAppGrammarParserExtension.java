...
import org.finos.legend.engine.language.pure.grammar.from.antlr4.SnowflakeAppLexerGrammar;
...
public class SnowflakeAppGrammarParserExtension implements PureGrammarParserExtension
{
    ...
    {
        SourceCodeParserInfo parserInfo = getSnowflakeAppInfo(sectionSourceCode);
        DefaultCodeSection section = new DefaultCodeSection();
        section.parserName = sectionSourceCode.sectionType;
        section.sourceInformation = parserInfo.sourceInformation;

        SnowflakeAppTreeWalker walker = new SnowflakeAppTreeWalker(parserInfo.input, parserInfo.walkerSourceInformation, elementConsumer, section);
        walker.visit((SnowflakeAppParserGrammar.DefinitionContext) parserInfo.rootContext);

        return section;
    }

    private static SourceCodeParserInfo getSnowflakeAppInfo(SectionSourceCode sectionSourceCode)
    {
        CharStream input = CharStreams.fromString(sectionSourceCode.code);
        ParserErrorListener errorListener = new ParserErrorListener(sectionSourceCode.walkerSourceInformation, SnowflakeAppLexerGrammar.VOCABULARY);
        SnowflakeAppLexerGrammar lexer = new SnowflakeAppLexerGrammar(input);
        lexer.removeErrorListeners();
        lexer.addErrorListener(errorListener);
        SnowflakeAppParserGrammar parser = new SnowflakeAppParserGrammar(new CommonTokenStream(lexer));
