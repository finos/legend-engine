...
public class ClassMappingSecondPassBuilder implements ClassMappingVisitor<SetImplementation>
{
    ...
    public SetImplementation visit(ClassMapping classMapping)
    {
        if (classMapping.extendsClassMappingId != null)
        {
            ...
            if (superSets.size() > 1)
            {
                throw new EngineException("Duplicated class mappings found with ID '" + superSetId + "' in mapping '" + HelperModelBuilder.getElementFullPath(parentMapping, this.context.pureModel.getExecutionSupport()) + "'", classMapping.sourceInformation, EngineErrorType.COMPILATION);
            }
        }
