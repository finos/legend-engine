...
public abstract class DataSourceSpecification
{
    ...
    protected DataSourceSpecification(DataSourceSpecificationKey key, DatabaseManager databaseManager, AuthenticationStrategy authenticationStrategy, @Deprecated  Properties extraUserProperties, RelationalExecutorInfo relationalExecutorInfo)
    {
        this.key = key;
        this.databaseManager = databaseManager;
        this.authenticationStrategy = authenticationStrategy;
        this.properties.putAll(extraUserProperties);
        this.connectionKey = new ConnectionKey(key, authenticationStrategy.getKey());

        relationalExecutorInfo.setDataSourceSpecifications(dataSourceSpecifications);

        synchronized (DataSourceSpecification.class) {
            dataSourceSpecifications.put(this.connectionKey.shortId(), this);
        }

        this.properties.put(DATASOURCE_SPEC_INSTANCE, this.connectionKey.shortId());
    ...
    protected Connection getConnection(Subject subject, MutableList<CommonProfile> profiles, String principal, Function0<DataSourceWithStatistics> exec) {
        try (Scope scope = GlobalTracer.get().buildSpan("Get Connection").startActive(true)) {
            ...
            // TODO : epsstan : refactor (connection leak ??)
            try
            {
                DriverWrapper driver = this.getDatabaseManager().getDriverWrapper();
                return driver.connect(this.buildJdbcURL(), this.buildJdbcDatasourceProperties());
            }
    ...
    protected HikariDataSource buildDataSourceImpl(MutableList<CommonProfile> profiles)
    {
        try (Scope scope = GlobalTracer.get().buildSpan("Create Pool").startActive(true)) {
            String currentUser = SubjectTools.getCurrentUsername() != null ? SubjectTools.getCurrentUsername() : this.authenticationStrategy.getAlternativePrincipal(profiles);
            String poolName = String.format("DBPool_%s_%s", this.connectionKey.shortId(), currentUser);

            Properties properties = new Properties();
            properties.putAll(this.buildJdbcDatasourceProperties());

            // TODO : epsstan : Why is the profile registered here
            ...
            jdbcConfig.setJdbcUrl(this.buildJdbcURL());
    ...
    public final Properties buildJdbcDatasourceProperties()
    {
        Properties properties = this.properties;
        Properties extraProperties = this.buildExtraJdbcDatasourceProperties();
    ...
    public abstract boolean isDatabaseIdentifiersCaseSensitive();
