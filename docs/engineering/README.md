...
| [SQL Dialect Translation](architecture/sql-dialect-translation.md) | The AST-based dialect framework (`SqlDialect`/`NodeProcessor`): how it differs from `sqlQueryToString`, end-to-end translation, declaring per-dialect divergence, and the SDT conformance suite |
| [SQL Optimizer (SQL Planning)](architecture/sql-optimizer.md) | The rule-based rewriter over the Postgres SQL AST (`planSqlStatement`): rule interface, fixed-point driver, the two shipped push-down rules, and the roadmap for db-specific rule modules and a wider rule catalogue |
