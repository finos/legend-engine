...
public class TestRelationalTimeZoneProcessing
{
    ...
    public void setUp()
    {
        ...
        try
        {
            ...
            exec("drop table if exists ProductSynonymTable", conn);
            exec("create table ProductSynonymTable (prodId Integer, name VARCHAR(200), synonym VARCHAR(200), type VARCHAR(10), createdBy VARCHAR(200), testDate DATE, testDateTime TIMESTAMP(9), from_z TIMESTAMP, thru_z TIMESTAMP)", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModA','GS-Mod-S1','STOCK','harted', \'2015-6-26\', \'2014-12-04 15:22:23.123456789\', \'2015-8-26 00:00:00\', \'2015-9-26 00:00:00\')", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModB','GS-Mod-S1','STOCK','harted', \'2015-7-26\', \'2014-12-04 23:22:23.123456789\', \'2015-8-26 23:00:00\', \'2015-9-26 00:00:00\')", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModC','GS-Mod-S1','STOCK','harted', \'2015-8-26\', \'2014-12-04 08:22:23\', \'2015-8-26 23:00:00\', \'2015-9-26 00:00:00\')", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModD','GS-Mod-S1','STOCK','harted', \'2015-9-26\', \'2014-12-04 08:22:23.123\', \'2015-8-26 23:00:00\', \'2015-9-26 00:00:00\')", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModE','GS-Mod-S1','STOCK','harted', \'2015-9-26\', \'2013-12-04 10:22:23\', \'2013-8-26 23:00:00\', \'2013-9-26 00:00:00\')", conn);
            exec("insert into ProductSynonymTable values(1, 'GS-ModF','GS-Mod-S1','STOCK','harted', \'2015-9-26\', \'2013-04-04 08:22:23.123\', \'2013-8-26 23:00:00\', \'2013-4-26 00:00:00\')", conn);
    ...
    private String templatePlanWithNoTz = "{\n" +
            "%templateFunctions%" +
            "  \"rootExecutionNode\": {\n" +
    ...
    {
        String planWithTz = templatePlanWithNoTz.replace("%timeZone%", ",\"timeZone\": \"GMT\"").replace("%templateFunctions%", getFreeMarkerAlloyDateFunction());
    ...
    {
        String planWithTz = templatePlanWithNoTz.replace("%timeZone%", ",\"timeZone\": \"US/Arizona\"").replace("%templateFunctions%", getFreeMarkerAlloyDateFunction());
        String plan = planWithTz.replace("%dateTimeParam%", "testDateTime = '2014-12-04 15:22:23.123456789'");
        SingleExecutionPlan executionPlan = objectMapper.readValue(plan, SingleExecutionPlan.class);
        RelationalResult result = (RelationalResult) executionPlan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(executionPlan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
    ...
    public void testNonDSTTTimeZonePlanWithDateTimeStringParameterPlan() throws IOException
    {
        ...
        String planWithTz = templatePlanWithNoTz.replace("%timeZone%", ",\"timeZone\": \"" + connectionTimeZone + "\"").replace("%templateFunctions%", getFreeMarkerAlloyDateFunction()).replace("%dateTimeParam%", "testDateTime = '${GMTtoTZ( \\\"[" + connectionTimeZone + "]\\\" dt )}'");
        SingleExecutionPlan executionPlan = objectMapper.readValue(planWithTz, SingleExecutionPlan.class);
        RelationalResult result = (RelationalResult) executionPlan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(vars, Lists.mutable.withAll(executionPlan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
    ...
    public void testTimeZoneRulesUsedAreAlignedWithTheDateParameter() throws IOException
    {
        ...
        vars.put("dt", new ConstantResult(gmtTestDateTimeParam));
        planWithTz = templatePlanWithNoTz.replace("%timeZone%", ",\"timeZone\": \"" + connectionTimeZone + "\"").replace("%templateFunctions%", getFreeMarkerAlloyDateFunction()).replace("%dateTimeParam%", "testDateTime = '${GMTtoTZ( \\\"[" + connectionTimeZone + "]\\\" dt )}'");
        executionPlan = objectMapper.readValue(planWithTz, SingleExecutionPlan.class);
        result = (RelationalResult) executionPlan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(vars, Lists.mutable.withAll(executionPlan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
    ...
    public void testGMTTimeZonePlanWithDateTimeStringParameter() throws IOException
    {
        ...
        String planWithTz = templatePlanWithNoTz.replace("%timeZone%", ",\"timeZone\": \"GMT\"").replace("%templateFunctions%", getFreeMarkerAlloyDateFunction()).replace("%dateTimeParam%", "testDateTime = '${dt}'");
        SingleExecutionPlan executionPlan = objectMapper.readValue(planWithTz, SingleExecutionPlan.class);
        RelationalResult result = (RelationalResult) executionPlan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(vars, Lists.mutable.withAll(executionPlan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
    ...
    public void testGMTTimeZonePlanWithDifferentSerializationFormats() throws IOException
    {
        ...
        RelationalResult result_pureTdsObject = (RelationalResult) plan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(plan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
        ...
        RelationalResult result_pureTds = (RelationalResult) plan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(plan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
        ...
        RelationalResult result_jsonTds = (RelationalResult) plan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(plan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
        ...
        RelationalResult result_csv = (RelationalResult) plan.rootExecutionNode.accept(new ExecutionNodeExecutor(null, new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(plan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
    ...
    {
        String planString = "{\"_type\":\"simple\",\"authDependent\":false,\"kerberos\":null,\"serializer\":{\"name\":\"pure\",\"version\":\"vX_X_X\"},\"templateFunctions\":[\"<#function renderCollection collection separator><#return collection?join(separator)></#function>\",\"<#function collectionSize collection> <#return collection?size> </#function>\"],\"rootExecutionNode\":{\"_type\":\"relationalTdsInstantiation\",\"resultType\":{\"_type\":\"tds\",\"tdsColumns\":[{\"name\":\"testDateTime\",\"type\":\"DateTime\",\"doc\":null,\"relationalType\":\"TIMESTAMP\",\"enumMapping\":{}}]},\"executionNodes\":[{\"_type\":\"sql\",\"resultType\":{\"_type\":\"dataType\",\"dataType\":\"meta::pure::metamodel::type::Any\"},\"executionNodes\":[],\"resultSizeRange\":null,\"implementation\":null,\"sqlQuery\":\"select \\\"root\\\".testDateTime as \\\"testDateTime\\\" from ProductSynonymTable as \\\"root\\\"\",\"onConnectionCloseCommitQuery\":null,\"onConnectionCloseRollbackQuery\":null,\"connection\":{\"_type\": \"RelationalDatabaseConnection\",\"type\": \"H2\",\"authenticationStrategy\" : {\"_type\" : \"test\"},\"datasourceSpecification\" : {\"_type\" : \"static\",\"databaseName\" : \"testDB\",\"host\":\"127.0.0.1\",\"port\" : \""+port+"\"},\"timeZone\":\"US/Arizona\"},\"resultColumns\":[{\"label\":\"\\\"testDateTime\\\"\",\"dataType\":\"TIMESTAMP\"}]}],\"resultSizeRange\":null,\"implementation\":null},\"globalImplementationSupport\":null}";
        SingleExecutionPlan plan = objectMapper.readValue(planString, SingleExecutionPlan.class);
