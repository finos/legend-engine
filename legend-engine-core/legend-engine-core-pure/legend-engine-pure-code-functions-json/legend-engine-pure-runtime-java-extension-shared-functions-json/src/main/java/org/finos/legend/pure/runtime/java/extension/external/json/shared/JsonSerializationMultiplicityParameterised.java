...
public class JsonSerializationMultiplicityParameterised<T> extends JsonPropertySerialization<T>
{
    public JsonSerializationMultiplicityParameterised(AbstractProperty property, boolean isFromAssociation, Conversion<T, Object> conversion, Type type)
    {
    ...
    public Object apply(RichIterable<T> pureObject, ConversionContext context)
    {
        ...
        for (T val : pureObject)
        {
            Object serializedValue = this.serialize(val, (JsonSerializationContext) context);
            if (serializedValue != CYCLE_DETECTED)
            {
