...
public class RegexpIndexOf extends NativeFunction
{
    private final ModelRepository repository;

    public RegexpIndexOf(FunctionExecutionInterpreted functionExecution, ModelRepository repository)
    {
    ...
    public CoreInstance execute(ListIterable<? extends CoreInstance> params, Stack<MutableMap<String, CoreInstance>> resolvedTypeParameters, Stack<MutableMap<String, CoreInstance>> resolvedMultiplicityParameters, VariableContext variableContext, MutableStack<CoreInstance> functionExpressionCallStack, Profiler profiler, InstantiationContext instantiationContext, ExecutionSupport executionSupport, Context context, ProcessorSupport processorSupport)
    {
        ...
        int groupNumber = NumericUtilities.toJavaNumber(Instance.getValueForMetaPropertyToOneResolved(params.get(2), M3Properties.values, processorSupport), processorSupport).intValue();
        ListIterable<? extends CoreInstance> enumerations = Instance.getValueForMetaPropertyToManyResolved(params.get(3), M3Properties.values, processorSupport);
        int patternFlags = 0;
        ...
        if (matcher.find())
        {
            startIndex = groupNumber == 0 ? matcher.start() :
                         groupNumber <= matcher.groupCount() ? matcher.start(groupNumber) : -1;
        }

        return NumericUtilities.toPureNumberValueExpression(startIndex, false, this.repository, processorSupport);
    }
}
