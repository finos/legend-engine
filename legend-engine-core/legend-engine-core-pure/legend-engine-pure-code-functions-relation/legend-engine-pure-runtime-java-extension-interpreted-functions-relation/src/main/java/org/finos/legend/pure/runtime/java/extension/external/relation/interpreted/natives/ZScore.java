...
public class ZScore extends Shared
{
    public ZScore(FunctionExecutionInterpreted functionExecution, ModelRepository repository)
    {
    ...
    ) throws PureExecutionException
    {
        ...
        CoreInstance inputColsSpec = Instance.getValueForMetaPropertyToOneResolved(params.get(1), M3Properties.values, processorSupport);
        CoreInstance outputColsSpec = Instance.getValueForMetaPropertyToOneResolved(params.get(2), M3Properties.values, processorSupport);
        CoreInstance windowSpec = null;
        if (params.size() > 3)
        {
            windowSpec = Instance.getValueForMetaPropertyToOneResolved(params.get(3), M3Properties.values, processorSupport);
        }

        TestTDS resultTDS = tds.zScore(inputColsSpec, outputColsSpec, windowSpec, processorSupport);
        ...
        return ValueSpecificationBootstrap.wrapValueSpecification(
                new TDSCoreInstance(resultTDS, returnGenericType, repository, processorSupport),
                false,
                processorSupport
        );
    }
