...
import static org.finos.legend.engine.test.mft.MFTTestSuitBuilder.isMFTTestCollection;

public class MFTTestCollection
{
    ...
    private final MutableList<MFTTestCollection> subCollections = Lists.mutable.with();
    ...
    public MFTTestCollection(String testCollectionName, CoreInstance pkg, String storeType, ProcessorSupport processorSupport)
    {
        this.pkg = pkg;
    ...
    public MFTTestCollection(CoreInstance pkg, String storeType, ProcessorSupport processorSupport)
    {
        this.pkg = pkg;
        this.storeType = storeType;
    ...
    public boolean hasTestContent()
    {
        return this.testFunctions.notEmpty() ||
                this.subCollections.anySatisfy(MFTTestCollection::hasTestContent);
    }
    ...
    private void collectAllTests(MutableList<CoreInstance> tests)
    {
        ...
        for (MFTTestCollection subCollection : this.subCollections)
        {
            subCollection.collectAllTests(tests);
        }
    }

    public RichIterable<MFTTestCollection> getSubCollections()
    {
    ...
    private void findPackageTests(ProcessorSupport processorSupport)
    {
        for (CoreInstance child : Instance.getValueForMetaPropertyToManyResolved(this.pkg, M3Properties.children, processorSupport))
        {
            if (Instance.instanceOf(child, M3Paths.FunctionDefinition, processorSupport))
            {
                   ...
                   if (isMFTTestCollection(child, processorSupport))
                    {
                        MFTTestCollection subCollection = new MFTTestCollection(child.getName(), child, storeType, processorSupport);
            ...
            {
                MFTTestCollection subCollection = new MFTTestCollection(child, storeType, processorSupport);
