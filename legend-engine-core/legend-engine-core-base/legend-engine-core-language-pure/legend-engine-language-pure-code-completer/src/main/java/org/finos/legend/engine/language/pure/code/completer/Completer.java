...
import org.finos.legend.engine.language.pure.code.completer.handlers.ExtendHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.FilterHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.FromHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.GroupByHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.JoinHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.RenameHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.SelectHandler;
import org.finos.legend.engine.language.pure.code.completer.handlers.SortHandler;
import org.finos.legend.engine.language.pure.code.completer.parser.ParserFixer;
...
{
    private static final String RUN_FUNCTION_QUALIFIED_PATH = "completer::__internal__::run__Any_MANY_";
    private static final String RUN_FUNCTION_SIGNATURE = "completer::__internal__::run():Any[*]";
    ...
    private final List<Function2<Object, PureModel, CompletionResult>> extraClassInstanceProcessors;
    ...
    public Completer(String buildCodeContext, List<CompleterExtension> extensions)
    {
        this(buildCodeContext, extensions, x -> Compiler.compile(x, null, Identity.getAnonymousIdentity().getName()));
    }

    public Completer(String buildCodeContext, List<CompleterExtension> extensions, java.util.function.Function<PureModelContextData, PureModel> compiler)
    {
        this.compiler = compiler;
        this.extraClassInstanceProcessors = ListIterate.flatCollect(extensions, CompleterExtension::getExtraClassInstanceProcessors);
        this.buildCodeContext = buildCodeContext;
        this.header =
                buildCodeContext +
                        "\n###Pure\n" +
                        "import meta::pure::functions::relation::*;\n" +
                        "function " + RUN_FUNCTION_SIGNATURE + "{\n";
    ...
    private CompletionResult processClassInstance(ClassInstance topExpression, PureModel pureModel)
    {
        ...
        Optional<CompletionResult> result = this.extraClassInstanceProcessors.stream().map(x -> x.apply(islandExpr, pureModel)).filter(Objects::nonNull).findFirst();
        return result.orElseGet(() -> new CompletionResult(Lists.mutable.empty()));
    }
    ...
    private ValueSpecification parseValueSpecification(String value)
    {
        ...
        Function func = (Function) ListIterate.select(pureModelContextData.getElements(), s -> s.getPath().equals(RUN_FUNCTION_QUALIFIED_PATH)).getFirst();
