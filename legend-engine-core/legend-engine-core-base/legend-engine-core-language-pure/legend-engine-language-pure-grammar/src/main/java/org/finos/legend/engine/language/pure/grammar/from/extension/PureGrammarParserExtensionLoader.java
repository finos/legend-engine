...
{
    private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(PureGrammarParserExtensionLoader.class);
    private static final AtomicReference<List<PureGrammarParserExtension>> INSTANCE = new AtomicReference<>();

    public static void logExtensionList()
    {
        if (LOGGER.isDebugEnabled())
        {
            LOGGER.debug(LazyIterate.collect(extensions(), extension -> "- " + extension.getClass().getSimpleName()).makeString("PureGrammarParserExtension loaded:\n", "\n", ""));
        }
    }
        ...
        return INSTANCE.updateAndGet(existing ->
        {
            if (existing == null)
            {
                ...
                for (PureGrammarParserExtension extension : ServiceLoader.load(PureGrammarParserExtension.class))
                {
                    ...
                    {
                        LOGGER.error("Failed to load PureGrammarParserExtension '" + extension.getClass().getSimpleName() + "'");
