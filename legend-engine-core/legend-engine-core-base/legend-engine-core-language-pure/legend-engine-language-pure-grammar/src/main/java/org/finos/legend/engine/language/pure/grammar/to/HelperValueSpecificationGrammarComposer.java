...
public class HelperValueSpecificationGrammarComposer
{
    ...
    public static String printType(Type type, DEPRECATED_PureGrammarComposerCore transformer)
    {
        ...
        {
            return "(" + ListIterate.collect(((RelationType) type).columns, x -> PureGrammarComposerUtility.convertIdentifier(x.name, transformer.identifierConverter()) + ":" + printGenericType(x.genericType, transformer) + (x.multiplicity == null || Multiplicity.ZERO_ONE.equals(x.multiplicity) ? "" : "[" + HelperDomainGrammarComposer.renderMultiplicity(x.multiplicity) + "]")).makeString(", ") + ")";
        }
