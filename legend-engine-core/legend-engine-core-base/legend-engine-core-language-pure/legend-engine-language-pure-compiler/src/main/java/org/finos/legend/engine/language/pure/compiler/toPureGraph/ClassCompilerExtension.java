    ...
    public Iterable<? extends Processor<?>> getExtraProcessors()
    {
        return Lists.fixedSize.of(
                Processor.newProcessor(
                        ...
                        this::classFirstPass,
                        this::classSecondPass,
                        this::classThirdPass
                )
        );
    }

    private PackageableElement classFirstPass(Class srcClass, CompileContext context)
    {
        ...
        GenericType genericType = context.newGenericType(targetClass);;
        context.pureModel.typesGenericTypeIndex.put(fullPath, genericType);
        return targetClass._classifierGenericType(context.newGenericType(context.pureModel.getType("meta::pure::metamodel::type::Class"), genericType))
                ._stereotypes(ListIterate.collect(srcClass.stereotypes, context::resolveStereotype))
                ._taggedValues(ListIterate.collect(srcClass.taggedValues, context::newTaggedValue));
    }

    @SuppressWarnings({"unchecked", "rawtypes"})
    private void classSecondPass(Class srcClass, CompileContext context)
    {
        String fullPath = context.pureModel.buildPackageString(srcClass._package, srcClass.name);
