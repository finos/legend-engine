    ...
    public Iterable<? extends Processor<?>> getExtraProcessors()
    {
        return Lists.fixedSize.of(
                Processor.newProcessor(
                        Enumeration.class,
                        (Enumeration enumeration, CompileContext context) ->
                        {
                            ...
                            GenericType genericType = HelperCoreBuilder.newGenericType(en, context);
                            context.pureModel.typesGenericTypeIndex.put(fullPath, genericType);
                            return en._classifierGenericType(HelperCoreBuilder.newGenericType(context.pureModel.getClass("meta::pure::metamodel::type::Enumeration"), genericType, context))
                                    ._stereotypes(ListIterate.collect(enumeration.stereotypes, stereotype -> HelperCoreBuilder.resolveStereotype(stereotype, context)))
                                    ._taggedValues(ListIterate.collect(enumeration.taggedValues, taggedValue -> HelperCoreBuilder.newTaggedValue(taggedValue, context)))
                                    ._values(ListIterate.collect(enumeration.values, v -> new Root_meta_pure_metamodel_type_Enum_Impl(v.value, SourceInformationHelper.toM3SourceInformation(v.sourceInformation), null)
                                            ...
                                            ._stereotypes(ListIterate.collect(v.stereotypes, stereotype -> HelperCoreBuilder.resolveStereotype(stereotype, context)))
                                            ._taggedValues(ListIterate.collect(v.taggedValues, taggedValue -> HelperCoreBuilder.newTaggedValue(taggedValue, context)))
                                            ._name(v.value)));
                        },
                        (Enumeration enumeration, CompileContext context) ->
                        {
