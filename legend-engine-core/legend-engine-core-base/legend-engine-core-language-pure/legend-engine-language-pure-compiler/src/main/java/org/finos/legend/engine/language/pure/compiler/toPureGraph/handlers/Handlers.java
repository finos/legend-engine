...
{
    private static final Map<String, MutableSet<String>> taxoMap = loadTaxonomyMap();
...
//                ps -> Lists.fixedSize.of(ps.get(0)._genericType()),
    ...
    private static Map<String, MutableSet<String>> loadTaxonomyMap()
    {
        MutableMap<String, MutableSet<String>> taxoMap = Maps.mutable.empty();

        CompilerExtensions.fromAvailableExtensions().getExtraSubTypesForFunctionMatching().forEach((key, value) ->
        {
            taxoMap.getIfAbsentPut(key, Sets.mutable::empty).addAll(value);
        });
