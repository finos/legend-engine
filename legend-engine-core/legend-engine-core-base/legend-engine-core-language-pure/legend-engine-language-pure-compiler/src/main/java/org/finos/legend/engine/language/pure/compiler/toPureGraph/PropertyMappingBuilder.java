...
public class PropertyMappingBuilder implements PropertyMappingVisitor<org.finos.legend.pure.m3.coreinstance.meta.pure.mapping.PropertyMapping>
{
    ...
    {
        // This path handles XStore property mappings with explicit source/target IDs
        ...
        InstanceSetImplementation targetSet = (InstanceSetImplementation) allClassMappings.detect(c -> c._id().equals(propertyMappingTargetId));
        if (targetSet == null)
        {
        ...
        return visitXStorePropertyMapping(propertyMapping, sourceSet, targetSet);
    }
    ...
    public PropertyMapping visitModelJoinPropertyMapping(org.finos.legend.engine.protocol.pure.v1.model.packageableElement.mapping.modelJoin.ModelJoinPropertyMapping propertyMapping, InstanceSetImplementation sourceSet, InstanceSetImplementation targetSet)
    {
        ...
        LambdaFunction lambda = compileBooleanLambda(sourceSet, targetSet, propertyMapping.joinCondition.body, propertyMapping.property.property, "ModelJoin property mapping function should return 'Boolean[1]'", HelperMappingBuilder.MODEL_JOIN_SOURCE_VAR, HelperMappingBuilder.MODEL_JOIN_TARGET_VAR);
    ...
    /**
     * Compiles a single XStorePropertyMapping with explicit source and target InstanceSetImplementations.
     */
    public PropertyMapping visitXStorePropertyMapping(org.finos.legend.engine.protocol.pure.v1.model.packageableElement.mapping.xStore.XStorePropertyMapping propertyMapping, InstanceSetImplementation sourceSet, InstanceSetImplementation targetSet)
    {
        LambdaFunction lambda = compileBooleanLambda(sourceSet, targetSet, propertyMapping.crossExpression.body, propertyMapping.property.property, "XStore property mapping function should return 'Boolean[1]'", "this", "that");
    ...
    // --- Shared helper methods for XStore and ModelJoin association property mapping resolution ---

    /**
     * Compiles a boolean lambda expression from the given body, using $this/$that variables typed
     * from the source/target set implementations. Validates the expression returns Boolean[1].
     */
    private LambdaFunction compileBooleanLambda(InstanceSetImplementation sourceSet, InstanceSetImplementation targetSet,
                                                 List<org.finos.legend.engine.protocol.pure.m3.valuespecification.ValueSpecification> body,
                                                 String propertyName, String errorMessage, String sourceVarName, String targetVarName)
    {
        ProcessingContext ctx = new ProcessingContext("Compile Boolean Lambda");
        ...
        org.finos.legend.pure.m3.coreinstance.meta.pure.metamodel.valuespecification.VariableExpression thisVariable = new Root_meta_pure_metamodel_valuespecification_VariableExpression_Impl("", null, context.pureModel.getClass("meta::pure::metamodel::valuespecification::VariableExpression"))._name(sourceVarName);
        thisVariable._genericType(context.newGenericType(thisClass));
        thisVariable._multiplicity(oneMultiplicity);

        org.finos.legend.pure.m3.coreinstance.meta.pure.metamodel.valuespecification.VariableExpression thatVariable = new Root_meta_pure_metamodel_valuespecification_VariableExpression_Impl("", null, context.pureModel.getClass("meta::pure::metamodel::valuespecification::VariableExpression"))._name(targetVarName);
        ...
        ctx.addInferredVariables(sourceVarName, thisVariable);
        ctx.addInferredVariables(targetVarName, thatVariable);
        ...
        ctx.flushVariable(targetVarName);
        ctx.flushVariable(sourceVarName);
        ...
        {
            throw new EngineException(errorMessage, body.get(body.size() - 1).sourceInformation, EngineErrorType.COMPILATION);
        }
