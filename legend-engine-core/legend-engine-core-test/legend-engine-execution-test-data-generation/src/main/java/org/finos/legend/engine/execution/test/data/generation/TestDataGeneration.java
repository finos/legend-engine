...
public class TestDataGeneration
{
    ...
    public static Response executeTestDataGenerateWithSeed(Function<PureModel, LambdaFunction> functionFunc, Function0<PureModel> pureModelFunc, String mapping, Runtime runtime, ExecutionContext context, List<TableRowIdentifiers> tableRowIdentifiers, Boolean hashStrings, Map<String, Object> parameterNameValueMap, String clientVersion, MutableList<CommonProfile> profiles, String user, PlanExecutor planExecutor)
    {
        try
        {
            ...
            LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_TEST_DATA_START, "").toString());
            ...
            Root_meta_pure_runtime_Runtime pureRuntime = HelperRuntimeBuilder.buildPureRuntime(runtime, pureModel.getContext());
            ...
            {
                return manageTestDataResult(profiles, result, pureHashStrings, pureModel, LoggingEventType.GENERATE_TEST_DATA_ERROR);
            }
        }
        catch (Exception ex)
        {
            return ExceptionTool.exceptionManager(ex, LoggingEventType.GENERATE_TEST_DATA_ERROR, profiles);
        }
    }

    public static Response executeTestDataGenerateWithDefaultSeedUtil(Function<PureModel, LambdaFunction> functionFunc, Function0<PureModel> pureModelFunc, String mapping, Runtime runtime, ExecutionContext context, Boolean hashStrings, Map<String, Object> parameterNameValueMap, String clientVersion, MutableList<CommonProfile> profiles, String user, PlanExecutor planExecutor)
    {
        PureModel pureModel = pureModelFunc.value();
        Mapping pureMapping = pureModel.getMapping(mapping);
        Root_meta_pure_runtime_Runtime pureRuntime = HelperRuntimeBuilder.buildPureRuntime(runtime, pureModel.getContext());
        Root_meta_pure_runtime_ExecutionContext executionContext = context == null ? new Root_meta_pure_runtime_ExecutionContext_Impl("") : HelperValueSpecificationBuilder.processExecutionContext(context, pureModel.getContext());
        return executeTestDataGenerateWithDefaultSeed(functionFunc, () -> pureModel, pureMapping, pureRuntime, executionContext, hashStrings, parameterNameValueMap, clientVersion, profiles, user, planExecutor);
    }

    public static Response executeTestDataGenerateWithDefaultSeed(Function<PureModel, LambdaFunction> functionFunc, Function0<PureModel> pureModelFunc, Mapping pureMapping, Root_meta_pure_runtime_Runtime pureRuntime, Root_meta_pure_runtime_ExecutionContext executionContext, Boolean hashStrings, Map<String, Object> parameterNameValueMap, String clientVersion, MutableList<CommonProfile> profiles, String user, PlanExecutor planExecutor)
    {
        try
        {
            long start = System.currentTimeMillis();
            ...
            PureModel pureModel = pureModelFunc.value();
            LambdaFunction<?> lambdaFunction = functionFunc.valueOf(pureModel);
            LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_SEED_DATA_START, "").toString());

            RichIterable<? extends Root_meta_pure_extension_Extension> routerExtensions = PureCoreExtensionLoader.extensions().flatCollect(e -> e.extraPureCoreExtensions(pureModel.getExecutionSupport()));
            MutableList<PlanGeneratorExtension> planGeneratorExtensions = org.eclipse.collections.api.factory.Lists.mutable.withAll(ServiceLoader.load(PlanGeneratorExtension.class));
            MutableList<PlanTransformer> planTransformers = planGeneratorExtensions.flatCollect(PlanGeneratorExtension::getExtraPlanTransformers);

            Root_meta_pure_executionPlan_ExecutionPlan seedPlan = SeedDataGeneration.generateSeedDataPlan(lambdaFunction, pureMapping, pureRuntime, executionContext, parameterNameValueMap, pureModel, routerExtensions, profiles);
            Result seedDataResult = planExecutor.execute(PlanGenerator.transformExecutionPlan(seedPlan, pureModel, clientVersion, profiles, routerExtensions, planTransformers), Maps.mutable.empty(), user, profiles);
            ...
            if (seedDataResult instanceof ErrorResult)
            {
                ...
                LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_SEED_DATA_ERROR, message).toString());
                return manageTestDataResult(profiles, seedDataResult, false, pureModel, LoggingEventType.GENERATE_TEST_DATA_ERROR);
            }
            ...
            LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_SEED_DATA_STOP, System.currentTimeMillis() - start).toString());
            boolean pureHashStrings = hashStrings == null ? false : hashStrings;
            Root_meta_pure_executionPlan_ExecutionPlan testDataPlan = generateTestDataPlan(lambdaFunction, pureMapping, pureRuntime, executionContext, defaultTableRowIdentifiers, pureHashStrings, parameterNameValueMap, pureModel, profiles);
            SingleExecutionPlan plan = PlanGenerator.transformExecutionPlan(testDataPlan, pureModel, clientVersion, profiles, routerExtensions, planTransformers);
            Result result = planExecutor.execute(plan, planExecutor.buildDefaultExecutionState(plan, Maps.mutable.empty()).setRealizeAllocationResults(true), user, profiles);
            LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_TEST_DATA_STOP, System.currentTimeMillis() - start).toString());
            try (Scope scope = GlobalTracer.get().buildSpan("Manage Results").startActive(true))
            {
    ...
    private static Root_meta_pure_executionPlan_ExecutionPlan generateTestDataPlan(LambdaFunction<?> lambdaFunction, Mapping mapping, Root_meta_pure_runtime_Runtime runtime, Root_meta_pure_runtime_ExecutionContext executionContext, MutableList<Root_meta_relational_testDataGeneration_TableRowIdentifiers> tableRowIdentifiers, boolean hashStrings, Map<String, Object> parameterNameValueMap, PureModel pureModel, MutableList<CommonProfile> profiles)
    {
        ...
        try (Scope scope = GlobalTracer.get().buildSpan("Generate Plan").startActive(true))
        {
            ...
            LOGGER.info(new LogInfo(profiles, LoggingEventType.PLAN_GENERATED, stringPlan).toString());
        }
        return plan;
    }

    public static Response manageTestDataResult(MutableList<CommonProfile> profiles, Result result, boolean hashStrings, PureModel pureModel, LoggingEventType loggingEventType)
    {
        if (result instanceof ErrorResult)
        {
            ...
            LOGGER.info(new LogInfo(profiles, loggingEventType, message).toString());
