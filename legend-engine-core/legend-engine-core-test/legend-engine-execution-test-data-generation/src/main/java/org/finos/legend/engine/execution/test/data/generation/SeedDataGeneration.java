...
public class SeedDataGeneration
{
    ...
    public static Response executeSeedDataGenerate(Function<PureModel, LambdaFunction> functionFunc, Function0<PureModel> pureModelFunc, String mapping, Runtime runtime, ExecutionContext context, Map<String, Object> parameterNameValueMap, String clientVersion, MutableList<CommonProfile> profiles, String user, PlanExecutor planExecutor)
    {
        try
        {
            ...
            SingleExecutionPlan plan = PlanGenerator.transformExecutionPlan(generateSeedDataPlan(lambdaFunction, pureMapping, pureRuntime, executionContext, parameterNameValueMap, pureModel, routerExtensions, profiles), pureModel, clientVersion, profiles, routerExtensions, planTransformers);
            Result result = planExecutor.execute(plan, Maps.mutable.empty(), user, profiles);
            LOGGER.info(new LogInfo(profiles, LoggingEventType.GENERATE_SEED_DATA_STOP, System.currentTimeMillis() - start).toString());
            return manageSeedDataResult(profiles, result, LoggingEventType.GENERATE_SEED_DATA_ERROR);
        }
        catch (Exception ex)
        {
            return ExceptionTool.exceptionManager(ex, LoggingEventType.GENERATE_SEED_DATA_ERROR, profiles);
        }
    }

    public static Root_meta_pure_executionPlan_ExecutionPlan generateSeedDataPlan(LambdaFunction<?> lambdaFunction, Mapping mapping, Root_meta_pure_runtime_Runtime runtime, Root_meta_pure_runtime_ExecutionContext executionContext, Map<String, Object> parameterNameValueMap, PureModel pureModel, RichIterable<? extends Root_meta_pure_extension_Extension> routerExtensions, MutableList<CommonProfile> profiles)
    {
    ...
    private static Response manageSeedDataResult(MutableList<CommonProfile> profiles, Result result, LoggingEventType loggingEventType)
    {
