...
public class PostgresWireProtocol {
    ...
    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(PostgresWireProtocol.class);
...
/*            // If we have deferred any executions we need to trigger a sync now because the client is expecting data
 ...
 /*       if (query.isEmpty()) {
    ...
    private void sendResultSet(Channel channel, String query, ResultSet rs, boolean isSimpleQuery ) throws SQLException {
        int rowCount = 0;
        if(rs != null ){
            if(isSimpleQuery){
