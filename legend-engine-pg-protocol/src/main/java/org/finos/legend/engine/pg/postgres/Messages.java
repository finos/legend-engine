...
public class Messages {
    ...
    static void sendDataRow(Channel channel, ResultSet rs, List<PGType> columnTypes, @Nullable FormatCodes.FormatCode[] formatCodes) throws SQLException {
        int length = 4 + 2;
        assert columnTypes.size() == rs.getMetaData().getColumnCount()
            : "Number of columns in the row must match number of columnTypes. Row: " + rs + " types: " + columnTypes;
...
*/
    ...
    static void sendRowDescription(Channel channel,
        ...
        ByteBuf buffer = channel.alloc().buffer(
            length + (resultSetMetaData.getColumnCount() * (10 + columnSize))); // use 10 as an estimate for columnName length
       ...
       /*     if (column instanceof Reference ref) {
            ...
            PGType<?> pgType = PGTypes.get(resultSetMetaData.getColumnType(idx+1), resultSetMetaData.getScale(idx+1));
