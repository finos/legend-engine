...
public class CompositeClassPathFilter implements ClassPathFilter
{
    ...
    public CompositeClassPathFilter(List<ClassPathFilter> delegates)
    {
    ...
    {
        return ListIterate.anySatisfy(delegates, d -> d.isPermittedPackage(packageName));
    }

    @Override
    public boolean isPermittedClass(String packageName, String className)
    {
        return ListIterate.anySatisfy(delegates, d -> d.isPermittedClass(packageName, className));
    }
}
