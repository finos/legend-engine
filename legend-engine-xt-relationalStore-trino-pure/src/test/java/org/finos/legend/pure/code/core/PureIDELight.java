...
public class PureIDELight extends PureIDEServer
{
    ...
    {
        new PureIDELight().run(args.length == 0 ? new String[]{"server", "legend-engine-xt-relationalStore-trino-pure/src/test/resources/ideLightConfig.json"} : args);
    }

    @Override
    protected MutableList<RepositoryCodeStorage> buildRepositories(SourceLocationConfiguration sourceLocationConfiguration)
    {
        String trinoRepo = "legend-engine-xt-relationalStore-trino-pure/src/main/resources/core_relational_trino";
        return Lists.mutable.<RepositoryCodeStorage>empty()
                .with(new MutableFSCodeStorage(GenericCodeRepository.build(Paths.get(trinoRepo + ".definition.json")), Paths.get(trinoRepo)));
    }
