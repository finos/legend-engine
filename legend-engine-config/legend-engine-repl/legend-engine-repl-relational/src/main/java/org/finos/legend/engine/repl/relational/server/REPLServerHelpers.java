...
public class REPLServerHelpers
{
    ...
    public static class REPLServerState
    {
        ...
        public void initializeWithREPLExecutedQuery(Execute.ExecuteResult executeResult)
        {
            ...
            try
            {
                isDynamic = ((SQLExecutionNode) executeResult.plan.rootExecutionNode.executionNodes.get(0)).isResultColumnsDynamic;
            }
            catch (Exception e)
            {
                // do nothing
            ...
            if (isDynamic)
            {
                throw new RuntimeException("Can't initialize DataCube. Last executed query produced dynamic result, add ->cast() then try again or use 'cache' command to dump the data out to a table and query against that table instead...");
            }
