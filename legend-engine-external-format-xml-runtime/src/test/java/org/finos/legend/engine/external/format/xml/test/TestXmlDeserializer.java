...
public class TestXmlDeserializer
{
    ...
    public void testDeserializeOneFirmWithAttributes()
    {
        ...
        Assert.assertEquals(Arrays.asList("Acme Inc."), firms.stream().map(IChecked::getValue).map(Firm::getName).collect(Collectors.toList()));
        Assert.assertEquals(Arrays.asList(2L), firms.stream().map(IChecked::getValue).map(Firm::getRanking).collect(Collectors.toList()));

        List<XmlDataRecord> sources = firms.stream().map(IChecked::getSource).map(XmlDataRecord.class::cast).collect(Collectors.toList());
        Assert.assertEquals(Arrays.asList("<firm name=\"Acme Inc.\" ranking=\"2\"/>"), sources.stream().map(XmlDataRecord::getRecord).collect(Collectors.toList()));
        Assert.assertEquals(Arrays.asList(1L), sources.stream().map(XmlDataRecord::getNumber).collect(Collectors.toList()));
    }
    ...
    public void testDeserializeMalformedDocument()
    {
        ...
        {
            Assert.assertEquals("Unexpected close tag </soft>; expected </firm>.\n at [row,col,system-id]: [6,10,\"executor:test\"]", e.getMessage());
        }
    }
    ...
    public void testDeserializeFirmWithEmployeesWithAttributes()
    {
        ...
        List<XmlDataRecord> sources = firms.stream().map(IChecked::getSource).map(XmlDataRecord.class::cast).collect(Collectors.toList());
        Assert.assertEquals(Arrays.asList("<firm name=\"Acme Inc.\" ranking=\"2\"><employees firstName=\"John\" lastName=\"Doe\" dateOfBirth=\"1991-02-10\" isAlive=\"true\" heightInMeters=\"1.78\"/><employees firstName=\"Fred\" lastName=\"Bloggs\" dateOfBirth=\"1983-12-13\" isAlive=\"false\" heightInMeters=\"1.65\"/></firm>"), sources.stream().map(XmlDataRecord::getRecord).collect(Collectors.toList()));
        Assert.assertEquals(Arrays.asList(1L), sources.stream().map(XmlDataRecord::getNumber).collect(Collectors.toList()));
    }
