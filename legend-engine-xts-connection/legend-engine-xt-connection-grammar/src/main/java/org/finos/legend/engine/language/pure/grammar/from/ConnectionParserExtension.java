...
import org.finos.legend.engine.language.pure.grammar.from.antlr4.ConnectionLexerGrammar;
...
public class ConnectionParserExtension implements PureGrammarParserExtension
{
    ...
    {
        return Lists.immutable.with(SectionParser.newParser(NAME, ConnectionParserExtension::parseSection));
    }
    ...
    {
        SourceCodeParserInfo parserInfo = getConnectionParserInfo(sectionSourceCode);
        DefaultCodeSection section = new DefaultCodeSection();
        section.parserName = sectionSourceCode.sectionType;
        section.sourceInformation = parserInfo.sourceInformation;
        ConnectionParseTreeWalker walker = new ConnectionParseTreeWalker(parserInfo.input, parserInfo.walkerSourceInformation, elementConsumer, section);
        walker.visit((ConnectionParserGrammar.DefinitionContext) parserInfo.rootContext);
        return section;
    }

    private static SourceCodeParserInfo getConnectionParserInfo(SectionSourceCode sectionSourceCode)
    {
