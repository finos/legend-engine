...
import org.finos.legend.engine.external.format.json.specifications.openapiv3_0_3.model.OpenAPIv3_0_3SchemaEmpty;
...
public class OpenAPIv3_0_3SchemaComposerVisitor implements JSONSchemaVisitor<OpenAPIv3_0_3Schema>
{
    ...
    @Override
    public OpenAPIv3_0_3SchemaEmpty visit(JSONSchemaEmpty schemaObject)
    {
        return (OpenAPIv3_0_3SchemaEmpty) loadCommonProperties(schemaObject, OpenAPIv3_0_3SchemaEmpty.builder()).build();
    ...
    public OpenAPIv3_0_3SchemaInteger visit(JSONSchemaInteger schemaObject)
    {
        ...
        if (schemaObject.exclusiveMaximum instanceof Boolean || schemaObject.exclusiveMinimum instanceof Boolean)
        {
            if (schemaObject.exclusiveMaximum instanceof Boolean)
            {
                userSchemaBuilder = userSchemaBuilder
                        .maximum(schemaObject.maximum)
                        .minimum(schemaObject.minimum)
                        .exclusiveMaximum((Boolean) schemaObject.exclusiveMaximum);
            }
            if (schemaObject.exclusiveMinimum instanceof Boolean)
            {
                userSchemaBuilder = userSchemaBuilder
                        .maximum(schemaObject.maximum)
                        .minimum(schemaObject.minimum)
                        .exclusiveMinimum((Boolean) schemaObject.exclusiveMinimum);
            }
        }
        else if (schemaObject.exclusiveMinimum instanceof Number || schemaObject.exclusiveMaximum instanceof Number)
        {
            if (schemaObject.exclusiveMinimum instanceof Number)
            {
                userSchemaBuilder = userSchemaBuilder
                        .exclusiveMinimum(true)
                        .minimum((Number) schemaObject.exclusiveMinimum);
            }
            if (schemaObject.exclusiveMaximum instanceof Number)
            {
                userSchemaBuilder = userSchemaBuilder
                        .exclusiveMaximum(true)
                        .minimum((Number) schemaObject.exclusiveMaximum);
            }
        }
        else
        {
