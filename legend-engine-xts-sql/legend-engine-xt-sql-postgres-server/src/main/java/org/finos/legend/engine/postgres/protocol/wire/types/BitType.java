...
public class BitType extends PGType<String>
{
    ...
    @Override
    public int writeAsBinary(ByteBuf buffer, String value2)
    {
        BitString value = BitString.ofRawBits(value2);
    ...
    public String readBinaryValue(ByteBuf buffer, int valueLength)
    {
        ...
        return new BitString(bitSet, length).asRawBitString();
    }
    ...
    String decodeUTF8Text(byte[] bytes)
    {
        ...
        {
            return BitString.ofBitString(text).asRawBitString();
        }
        else
        {
            if (length > 0)
            {
                return BitString.ofRawBits(text, length).asRawBitString();
            }
            else
            {
                return BitString.ofRawBits(text).asRawBitString();
            }
        }
    }
}
