...
import org.finos.legend.engine.persistence.components.e2e.versioning.TestDedupAndVersioning;
...
{
    private final String basePathForInput = "data/unitemporal-incremental-milestoning/input/batch_id_based/";
    ...
    */
    ...
    void testMilestoningWithLessColumnsInStaging() throws Exception
    {
        ...
        executePlansAndVerifyResults(ingestMode, options, datasets.withStagingDataset(stagingTable), schema, expectedDataPass2, expectedStats, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");

        // ------------ Perform Pass3 empty batch (No Impact) -------------------------
        String dataPass3 = "data/empty_file.csv";
        String expectedDataPass3 = basePathForExpected + "less_columns_in_staging/expected_pass3.csv";
        loadBasicStagingDataWithLessColumnsThanMain(dataPass3);

        // Execute plans and verify results
        expectedStats = createExpectedStatsMap(0, 0, 0, 0, 0);
        executePlansAndVerifyResults(ingestMode, options, datasets.withStagingDataset(stagingTable), schema, expectedDataPass3, expectedStats, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");
    }
    ...
    */
    ...
    void testMilestoningAllVersionWithoutPerform() throws Exception
    {
        ...
        executePlansAndVerifyResultsWithSpecifiedDataSplits(ingestMode, options, datasets, schema, expectedDataPass1, expectedStatsList, dataSplitRanges, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");
        ...
        executePlansAndVerifyResultsWithSpecifiedDataSplits(ingestMode, options, Datasets.of(mainTable, stagingTable), schema, expectedDataPass2, expectedStatsList, dataSplitRanges, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");

        // ------------ Perform milestoning Pass3 - Empty batch ------------------------
        String dataPass3 = "data/empty_file.csv";
        loadBasicStagingDataWithDataSplit(dataPass3);
        ...
        executePlansAndVerifyResultsWithSpecifiedDataSplits(ingestMode, options, Datasets.of(mainTable, stagingTable), schema, expectedDataPass3, expectedStatsList, dataSplitRanges, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");
    }
    ...
    void testUniTemporalDeltaWithAllVersionGreaterThanAndStagingFilters() throws Exception
    {
        ...
        executePlansAndVerifyResultsWithDerivedDataSplits(ingestMode, options, datasets, schema, expectedDataPass1, expectedStatsList, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");
        ...
        stagingTable = stagingTable.withDatasetFilters(DatasetFilter.of("batch", FilterType.EQUAL_TO, 2));
        datasets = Datasets.of(mainTable, stagingTable);
        executePlansAndVerifyResultsWithDerivedDataSplits(ingestMode, options, datasets, schema, expectedDataPass2, expectedStatsList, fixedClock_2000_01_01, " order by \"batch_id_in\", \"id\"");
