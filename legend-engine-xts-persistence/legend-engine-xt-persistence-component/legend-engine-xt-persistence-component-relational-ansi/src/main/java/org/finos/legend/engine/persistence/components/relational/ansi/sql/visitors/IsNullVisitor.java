...
public class IsNullVisitor implements LogicalPlanVisitor<IsNull>
{

    @Override
    public VisitorResult visit(PhysicalPlanNode prev, IsNull current, VisitorContext context)
    {
        IsNullCondition isNullCondition = new IsNullCondition();
        prev.push(isNullCondition);

        return new VisitorResult(isNullCondition, Arrays.asList(current.value()));
    }
}
