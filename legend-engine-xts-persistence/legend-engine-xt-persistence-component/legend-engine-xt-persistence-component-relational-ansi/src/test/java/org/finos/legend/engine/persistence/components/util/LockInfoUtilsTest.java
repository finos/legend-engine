...
public class LockInfoUtilsTest
{
    ...
    public void testInitializeLockInfo()
    {
        ...
        String expectedSql = "INSERT INTO main_table_lock (\"insert_ts_utc\") (SELECT '2000-01-01 00:00:00' WHERE NOT (EXISTS (SELECT * FROM main_table_lock as main_table_lock)))";
    ...
    public void testUpdateMetaStore()
    {
        ...
        SqlPlan physicalPlan = transformer.generatePhysicalPlan(logicalPlan);
        List<String> list = physicalPlan.getSqlList();
        String expectedSql = "UPDATE main_table_lock as main_table_lock SET main_table_lock.\"insert_ts_utc\" = '2000-01-01 00:00:00'";
        Assertions.assertEquals(expectedSql, list.get(0));
    }
}
