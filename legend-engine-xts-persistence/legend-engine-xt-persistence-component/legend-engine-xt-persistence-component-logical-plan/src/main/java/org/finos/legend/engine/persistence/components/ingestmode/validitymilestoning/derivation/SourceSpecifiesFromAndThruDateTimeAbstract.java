...
public interface SourceSpecifiesFromAndThruDateTimeAbstract extends ValidityDerivation
{
    @Parameter(order = 0)
    String sourceDateTimeFromField();

    @Parameter(order = 1)
    String sourceDateTimeThruField();

    @Override
    default <T> T accept(ValidityDerivationVisitor<T> visitor)
    {
        return visitor.visitSourceSpecifiesFromAndThruDateTime(this);
    }
}
