...
public class StagedFilesDatasetReferenceVisitor implements LogicalPlanVisitor<StagedFilesDatasetReference>
{
    ...
    {
        StagedFilesTable stagedFiles = new StagedFilesTable(current.location());
        current.fileFormat().ifPresent(stagedFiles::setFileFormat);
        current.filePattern().ifPresent(stagedFiles::setFilePattern);
