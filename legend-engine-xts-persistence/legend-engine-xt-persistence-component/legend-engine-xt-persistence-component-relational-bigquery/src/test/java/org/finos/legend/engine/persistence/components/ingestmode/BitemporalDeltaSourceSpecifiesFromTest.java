...
import static org.finos.legend.engine.persistence.components.ingestmode.BigQueryTestArtifacts.getDropTempTableQuery;
...
public class BitemporalDeltaSourceSpecifiesFromTest extends BitemporalDeltaSourceSpecifiesFromTestCases
{
    ...
    public void verifyBitemporalDeltaBatchIdBasedNoDeleteIndNoDataSplits(GeneratorResult operations)
    {
        ...
        String expectedStageToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date` " +
                "FROM " +
    ...
    {
        String expectedStageToTemp = "INSERT INTO `mydb`.`temp` " +
                "(`id`, `name`, `amount`, `version`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`) " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_reference` as `legend_persistence_start_date`,legend_persistence_y.`legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
                "FROM " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest`,stage.`version`,stage.`data_split` FROM `mydb`.`staging` as stage WHERE (stage.`data_split` >= '{DATA_SPLIT_LOWER_BOUND_PLACEHOLDER}') AND (stage.`data_split` <= '{DATA_SPLIT_UPPER_BOUND_PLACEHOLDER}')) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),MIN(legend_persistence_x.`legend_persistence_end_date`)) as `legend_persistence_end_date` " +
                "FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date` " +
        ...
        String expectedMainToTemp = "INSERT INTO `mydb`.`temp` " +
                "(`id`, `name`, `amount`, `version`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`) " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,legend_persistence_y.`legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
                "FROM " +
                "(SELECT sink.`id`,sink.`name`,sink.`amount`,sink.`digest`,sink.`version`,sink.`batch_id_in`,sink.`batch_id_out`,sink.`validity_from_target`,sink.`validity_through_target` FROM `mydb`.`main` as sink WHERE sink.`batch_id_out` = 999999999) as legend_persistence_x " +
                "INNER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,legend_persistence_x.`legend_persistence_end_date` as `legend_persistence_end_date` " +
                "FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,MIN(legend_persistence_y.`legend_persistence_start_date`) as `legend_persistence_end_date` " +
        ...
        String expectedTempToMain = "INSERT INTO `mydb`.`main` " +
                "(`id`, `name`, `amount`, `digest`, `version`, `batch_id_in`, `batch_id_out`, `validity_from_target`, `validity_through_target`) " +
                "(SELECT temp.`id`,temp.`name`,temp.`amount`,temp.`digest`,temp.`version`,temp.`batch_id_in`,temp.`batch_id_out`,temp.`validity_from_target`,temp.`validity_through_target` FROM `mydb`.`temp` as temp)";

        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyMainTableWithVersionCreateQuery, operations.get(0).preActionsSql().get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, operations.get(0).preActionsSql().get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableWithVersionCreateQuery, operations.get(0).preActionsSql().get(2));
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithDeleteIndNoDataSplits(GeneratorResult operations)
    {
        ...
        String expectedTempToMainForDeletion = "INSERT INTO `mydb`.`main` " +
                ...
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`validity_from_target`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date`,legend_persistence_x.`batch_id_in`,legend_persistence_x.`batch_id_out` " +
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, preActionsSql.get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableCreateQuery, preActionsSql.get(2));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableWithDeleteIndicatorCreateQuery, preActionsSql.get(3));
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithDeleteIndWithDataSplits(List<GeneratorResult> operations, List<DataSplitRange> dataSplitRanges)
    {
        ...
        String expectedStageToTemp = "INSERT INTO " + tempName + " " +
                ...
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_reference` as `legend_persistence_start_date`,legend_persistence_y.`legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
                "FROM " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest`,stage.`version`,stage.`delete_indicator`,stage.`data_split` FROM `mydb`.`staging` as stage WHERE (stage.`delete_indicator` NOT IN ('yes','1','true')) AND ((stage.`data_split` >= '{DATA_SPLIT_LOWER_BOUND_PLACEHOLDER}') AND (stage.`data_split` <= '{DATA_SPLIT_UPPER_BOUND_PLACEHOLDER}'))) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),MIN(legend_persistence_x.`legend_persistence_end_date`)) as `legend_persistence_end_date` " +
                "FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date` " +
        ...
        String expectedTempToMain = "INSERT INTO `mydb`.`main` " +
                "(`id`, `name`, `amount`, `digest`, `version`, `batch_id_in`, `batch_id_out`, `validity_from_target`, `validity_through_target`) " +
                "(SELECT legend_persistence_temp.`id`,legend_persistence_temp.`name`,legend_persistence_temp.`amount`,legend_persistence_temp.`digest`,legend_persistence_temp.`version`,legend_persistence_temp.`batch_id_in`,legend_persistence_temp.`batch_id_out`,legend_persistence_temp.`validity_from_target`,legend_persistence_temp.`validity_through_target` FROM " + tempName + " as legend_persistence_temp)";

        String expectedMainToTempForDeletion = "INSERT INTO " + tempWithDeleteIndicatorName + " " +
                "(`id`, `name`, `amount`, `version`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`, `delete_indicator`) " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,legend_persistence_x.`validity_through_target` as `legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999,(CASE WHEN legend_persistence_y.`delete_indicator` IS NULL THEN 0 ELSE 1 END) " +
                "FROM " +
                "(SELECT * FROM `mydb`.`main` as sink WHERE (sink.`batch_id_out` = 999999999) " +
                "AND (EXISTS " +
        ...
        String expectedTempToMainForDeletion = "INSERT INTO `mydb`.`main` " +
                "(`id`, `name`, `amount`, `version`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`) " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`legend_persistence_start_date` as `legend_persistence_start_date`,MAX(legend_persistence_y.`validity_through_target`) as `legend_persistence_end_date`,legend_persistence_x.`batch_id_in`,legend_persistence_x.`batch_id_out` FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`validity_from_target`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date`,legend_persistence_x.`batch_id_in`,legend_persistence_x.`batch_id_out` " +
                ...
                "ON ((legend_persistence_x.`id` = legend_persistence_y.`id`) AND (legend_persistence_x.`name` = legend_persistence_y.`name`)) AND (legend_persistence_y.`validity_from_target` > legend_persistence_x.`validity_from_target`) AND (legend_persistence_y.`delete_indicator` = 0) " +
                "WHERE legend_persistence_x.`delete_indicator` = 0 " +
                "GROUP BY legend_persistence_x.`id`, legend_persistence_x.`name`, legend_persistence_x.`amount`, legend_persistence_x.`version`, legend_persistence_x.`digest`, legend_persistence_x.`validity_from_target`, legend_persistence_x.`batch_id_in`, legend_persistence_x.`batch_id_out`) as legend_persistence_x " +
                "LEFT OUTER JOIN " + tempWithDeleteIndicatorName + " as legend_persistence_y " +
                "ON ((legend_persistence_x.`id` = legend_persistence_y.`id`) AND (legend_persistence_x.`name` = legend_persistence_y.`name`)) AND (legend_persistence_y.`validity_through_target` > legend_persistence_x.`legend_persistence_start_date`) AND (legend_persistence_y.`validity_through_target` <= legend_persistence_x.`legend_persistence_end_date`) AND (legend_persistence_y.`delete_indicator` <> 0) " +
                "GROUP BY legend_persistence_x.`id`, legend_persistence_x.`name`, legend_persistence_x.`amount`, legend_persistence_x.`version`, legend_persistence_x.`digest`, legend_persistence_x.`legend_persistence_start_date`, legend_persistence_x.`batch_id_in`, legend_persistence_x.`batch_id_out`)";
    ...
    public void verifyBitemporalDeltaBatchIdBasedNoDeleteIndNoDataSplitsFilterDuplicates(GeneratorResult operations)
    {
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, preActionsSql.get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableCreateQuery, preActionsSql.get(2));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyStageWithoutDuplicatesTableCreateQuery, preActionsSql.get(3));
    ...
    {
        String expectedStageToStageWithoutDuplicates = "INSERT INTO `mydb`.`stagingWithoutDuplicates` " +
                "(`id`, `name`, `amount`, `validity_from_reference`, `digest`, `version`, `data_split`) " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest`,stage.`version`,stage.`data_split` FROM `mydb`.`staging` as stage " +
                "WHERE NOT (EXISTS (SELECT * FROM `mydb`.`main` as sink WHERE (sink.`digest` = stage.`digest`) AND (sink.`batch_id_out` = 999999999))))";
        ...
        String expectedStageToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_reference` as `legend_persistence_start_date`,legend_persistence_y.`legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
                "FROM " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest`,stage.`version`,stage.`data_split` FROM `mydb`.`stagingWithoutDuplicates` as stage WHERE (stage.`data_split` >= '{DATA_SPLIT_LOWER_BOUND_PLACEHOLDER}') AND (stage.`data_split` <= '{DATA_SPLIT_UPPER_BOUND_PLACEHOLDER}')) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),MIN(legend_persistence_x.`legend_persistence_end_date`)) as `legend_persistence_end_date` " +
                "FROM " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as `legend_persistence_end_date` " +
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, operations.get(0).preActionsSql().get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableWithVersionCreateQuery, operations.get(0).preActionsSql().get(2));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyStageWithVersionWithDataSplitWithoutDuplicatesTableCreateQuery, operations.get(0).preActionsSql().get(3));
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithDeleteIndNoDataSplitsFilterDuplicates(GeneratorResult operations)
    {
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableCreateQuery, preActionsSql.get(2));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableWithDeleteIndicatorCreateQuery, preActionsSql.get(3));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyStageWithDeleteIndicatorWithoutDuplicatesTableCreateQuery, preActionsSql.get(4));
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithDeleteIndWithDataSplitsFilterDuplicates(List<GeneratorResult> operations, List<DataSplitRange> dataSplitRanges)
    {
        ...
        String expectedStageToStageWithoutDuplicates = "INSERT INTO " + stageWithoutDuplicatesName + " " +
                "(`id`, `name`, `amount`, `validity_from_reference`, `digest`, `version`, `delete_indicator`, `data_split`) " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest`,stage.`version`,stage.`delete_indicator`,stage.`data_split` FROM `mydb`.`staging` as stage " +
                "WHERE NOT (EXISTS (SELECT * FROM `mydb`.`main` as sink WHERE (sink.`digest` = stage.`digest`) AND (sink.`batch_id_out` = 999999999))))";
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithPlaceholders(GeneratorResult operations)
    {
        ...
        Assertions.assertEquals(expectedUpdateMain, milestoningSql.get(2));
        Assertions.assertEquals(expectedTempToMain, milestoningSql.get(3));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableIngestQueryWithPlaceHolders, metadataIngestSql.get(0));
    }
    ...
    public void verifyBitemporalDeltaBatchIdAndTimeBasedNoDeleteIndNoDataSplits(GeneratorResult operations)
    {
        ...
        String expectedStageToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "legend_persistence_y.`legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999,PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59') " +
                "FROM " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest` FROM `mydb`.`staging` as stage) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),MIN(legend_persistence_x.`legend_persistence_end_date`)) as `legend_persistence_end_date` " +
        ...
        String expectedMainToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999,PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59') " +
                "FROM " +
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyMainTableBatchIdAndTimeBasedCreateQuery, preActionsSql.get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, preActionsSql.get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableBatchIdAndTimeBasedCreateQuery, preActionsSql.get(2));
    ...
    public void verifyBitemporalDeltaDateTimeBasedNoDeleteIndNoDataSplits(GeneratorResult operations)
    {
        ...
        String expectedStageToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_reference` as `legend_persistence_start_date`," +
                "legend_persistence_y.`legend_persistence_end_date`,PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59') " +
                "FROM " +
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`validity_from_reference`,stage.`digest` FROM `mydb`.`staging` as stage) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`legend_persistence_start_date`,COALESCE(MIN(legend_persistence_y.`legend_persistence_start_date`),MIN(legend_persistence_x.`legend_persistence_end_date`)) as `legend_persistence_end_date` " +
                ...
                "(SELECT `id`,`name`,`validity_from_reference` as `legend_persistence_start_date` FROM `mydb`.`staging` as stage) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT `id`,`name`,`validity_from_target` as `legend_persistence_start_date` FROM `mydb`.`main` as sink WHERE sink.`batch_time_out` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as legend_persistence_y " +
                "ON ((legend_persistence_x.`id` = legend_persistence_y.`id`) AND (legend_persistence_x.`name` = legend_persistence_y.`name`)) AND (legend_persistence_x.`legend_persistence_start_date` < legend_persistence_y.`legend_persistence_start_date`) " +
                "GROUP BY legend_persistence_x.`id`, legend_persistence_x.`name`, legend_persistence_x.`legend_persistence_start_date`) as legend_persistence_x " +
                "LEFT OUTER JOIN " +
                "(SELECT `id`,`name`,`validity_from_reference` as `legend_persistence_start_date` FROM `mydb`.`staging` as stage) as legend_persistence_y " +
        ...
        String expectedMainToTemp = "INSERT INTO `mydb`.`temp` " +
                ...
                "PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59') FROM (SELECT sink.`id`,sink.`name`,sink.`amount`,sink.`digest`,sink.`batch_time_in`," +
                "sink.`batch_time_out`,sink.`validity_from_target`,sink.`validity_through_target` " +
                "FROM `mydb`.`main` as sink WHERE sink.`batch_time_out` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59')) as legend_persistence_x " +
                "INNER JOIN " +
        ...
        String expectedUpdateMain = "UPDATE `mydb`.`main` as sink SET " +
                ...
                "(sink.`validity_from_target` = temp.`validity_from_target`))) AND (sink.`batch_time_out` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59'))";
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyMainTableDateTimeBasedCreateQuery, preActionsSql.get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMetadataTableCreateQuery, preActionsSql.get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedBitemporalFromOnlyTempTableDateTimeBasedCreateQuery, preActionsSql.get(2));
        ...
        Assertions.assertEquals(getExpectedMetadataTableIngestQuery(), metadataIngestSql.get(0));
        String incomingRecordCount = "SELECT COUNT(*) as `incomingRecordCount` FROM `mydb`.`staging` as stage";
        String rowsUpdated = "SELECT COUNT(*) as `rowsUpdated` FROM `mydb`.`main` as sink WHERE sink.`batch_time_out` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000')";
        String rowsInserted = "SELECT (SELECT COUNT(*) FROM `mydb`.`main` as sink WHERE sink.`batch_time_in` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'))-(SELECT COUNT(*) FROM `mydb`.`main` as sink WHERE sink.`batch_time_out` = PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000')) as `rowsInserted`";

        verifyStats(operations, incomingRecordCount, rowsUpdated, rowsDeleted, rowsInserted, rowsTerminated);
    }

    @Override
    public void verifyBitemporalDeltaBatchIdBasedWithTerminateIndNoDataSplits(GeneratorResult operations)
    {
        List<String> preActionsSql = operations.preActionsSql();
        List<String> milestoningSql = operations.ingestSql();
        List<String> metadataIngestSql = operations.metadataIngestSql();
        ...
        String expectedMainToTempForTermination = "INSERT INTO `mydb`.`tempWithDeleteIndicator` " +
            "(`id`, `name`, `amount`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`) " +
            "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,legend_persistence_y.`validity_from_reference` as `legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
            "FROM " +
            ...
            "INNER JOIN " +
            "(SELECT * FROM (SELECT *,ROW_NUMBER() OVER (PARTITION BY stage.`id`,stage.`name` ORDER BY stage.`validity_from_reference` DESC) as `legend_persistence_row_number` FROM (SELECT * FROM `mydb`.`staging` as stage WHERE stage.`delete_indicator` IN ('yes','1','true')) as stage) as stage WHERE stage.`legend_persistence_row_number` = 1) as legend_persistence_y " +
            "ON ((legend_persistence_x.`id` = legend_persistence_y.`id`) AND (legend_persistence_x.`name` = legend_persistence_y.`name`)) " +
            "AND (legend_persistence_y.`validity_from_reference` >= legend_persistence_x.`validity_from_target`))";

        String expectedUpdateMainForTermination = "UPDATE `mydb`.`main` as sink " +
            "SET sink.`batch_id_out` = (SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN')-1 " +
            "WHERE (EXISTS " +
            "(SELECT * FROM `mydb`.`tempWithDeleteIndicator` as tempWithDeleteIndicator " +
            "WHERE ((sink.`id` = tempWithDeleteIndicator.`id`) AND (sink.`name` = tempWithDeleteIndicator.`name`)) AND (sink.`validity_from_target` = tempWithDeleteIndicator.`validity_from_target`))) " +
        ...
        String expectedTempToMainForTermination = "INSERT INTO `mydb`.`main` " +
            "(`id`, `name`, `amount`, `digest`, `batch_id_in`, `batch_id_out`, `validity_from_target`, `validity_through_target`) " +
            "(SELECT tempWithDeleteIndicator.`id`,tempWithDeleteIndicator.`name`,tempWithDeleteIndicator.`amount`,tempWithDeleteIndicator.`digest`,tempWithDeleteIndicator.`batch_id_in`,tempWithDeleteIndicator.`batch_id_out`,tempWithDeleteIndicator.`validity_from_target`,tempWithDeleteIndicator.`validity_through_target` FROM `mydb`.`tempWithDeleteIndicator` as tempWithDeleteIndicator)";
        ...
        Assertions.assertEquals(expectedUpdateMain, milestoningSql.get(2));
        Assertions.assertEquals(expectedTempToMain, milestoningSql.get(3));
        Assertions.assertEquals(expectedMainToTempForTermination, milestoningSql.get(4));
        Assertions.assertEquals(expectedUpdateMainForTermination, milestoningSql.get(5));
        Assertions.assertEquals(expectedTempToMainForTermination, milestoningSql.get(6));
        Assertions.assertEquals(getExpectedCleanupSql("`mydb`.`temp`", "temp"), milestoningSql.get(7));
        Assertions.assertEquals(getExpectedCleanupSql("`mydb`.`tempWithDeleteIndicator`", "tempWithDeleteIndicator"), milestoningSql.get(8));

        Assertions.assertEquals(BigQueryTestArtifacts.getDropTempTableQuery("`mydb`.`temp`"), operations.postCleanupSql().get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.getDropTempTableQuery("`mydb`.`tempWithDeleteIndicator`"), operations.postCleanupSql().get(1));
    ...
    public void verifyBitemporalDeltaBatchIdBasedWithTerminateIndWithDataSplits(List<GeneratorResult> operations, List<DataSplitRange> dataSplitRanges)
    {
        ...
        String expectedMainToTempForTermination = "INSERT INTO " + tempWithDeleteIndicatorName + " " +
            "(`id`, `name`, `amount`, `version`, `digest`, `validity_from_target`, `validity_through_target`, `batch_id_in`, `batch_id_out`) " +
            "(SELECT legend_persistence_x.`id`,legend_persistence_x.`name`,legend_persistence_x.`amount`,legend_persistence_x.`version`,legend_persistence_x.`digest`,legend_persistence_x.`validity_from_target` as `legend_persistence_start_date`,legend_persistence_y.`validity_from_reference` as `legend_persistence_end_date`,(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN'),999999999 " +
            "FROM " +
            "(SELECT * FROM `mydb`.`main` as sink WHERE (sink.`batch_id_out` = 999999999) AND (sink.`validity_through_target` = '9999-12-31 23:59:59')) as legend_persistence_x " +
            "INNER JOIN " +
            "(SELECT * FROM (SELECT *,ROW_NUMBER() OVER (PARTITION BY stage.`id`,stage.`name` ORDER BY stage.`validity_from_reference` DESC) as `legend_persistence_row_number` FROM (SELECT * FROM `mydb`.`staging` as stage WHERE (stage.`delete_indicator` IN ('yes','1','true')) AND ((stage.`data_split` >= '{DATA_SPLIT_LOWER_BOUND_PLACEHOLDER}') AND (stage.`data_split` <= '{DATA_SPLIT_UPPER_BOUND_PLACEHOLDER}'))) as stage) as stage WHERE stage.`legend_persistence_row_number` = 1) as legend_persistence_y " +
            "ON ((legend_persistence_x.`id` = legend_persistence_y.`id`) AND (legend_persistence_x.`name` = legend_persistence_y.`name`)) " +
            "AND (legend_persistence_y.`validity_from_reference` >= legend_persistence_x.`validity_from_target`))";
        ...
        String expectedTempToMainForTermination = "INSERT INTO `mydb`.`main` " +
            "(`id`, `name`, `amount`, `digest`, `version`, `batch_id_in`, `batch_id_out`, `validity_from_target`, `validity_through_target`) " +
            "(SELECT legend_persistence_tempWithDeleteIndicator.`id`,legend_persistence_tempWithDeleteIndicator.`name`,legend_persistence_tempWithDeleteIndicator.`amount`,legend_persistence_tempWithDeleteIndicator.`digest`,legend_persistence_tempWithDeleteIndicator.`version`,legend_persistence_tempWithDeleteIndicator.`batch_id_in`,legend_persistence_tempWithDeleteIndicator.`batch_id_out`,legend_persistence_tempWithDeleteIndicator.`validity_from_target`,legend_persistence_tempWithDeleteIndicator.`validity_through_target` FROM " + tempWithDeleteIndicatorName + " as legend_persistence_tempWithDeleteIndicator)";
        ...
        Assertions.assertEquals(expectedUpdateMain, operations.get(0).ingestSql().get(2));
        Assertions.assertEquals(expectedTempToMain, operations.get(0).ingestSql().get(3));
        Assertions.assertEquals(enrichSqlWithDataSplits(expectedMainToTempForTermination, dataSplitRanges.get(0)), operations.get(0).ingestSql().get(4));
        Assertions.assertEquals(expectedUpdateMainForTermination, operations.get(0).ingestSql().get(5));
        Assertions.assertEquals(expectedTempToMainForTermination, operations.get(0).ingestSql().get(6));
        Assertions.assertEquals(getExpectedCleanupSql(tempName, "legend_persistence_temp"), operations.get(0).ingestSql().get(7));
        Assertions.assertEquals(getExpectedCleanupSql(tempWithDeleteIndicatorName, "legend_persistence_tempWithDeleteIndicator"), operations.get(0).ingestSql().get(8));
        ...
        Assertions.assertEquals(expectedUpdateMain, operations.get(1).ingestSql().get(2));
        Assertions.assertEquals(expectedTempToMain, operations.get(1).ingestSql().get(3));
        Assertions.assertEquals(enrichSqlWithDataSplits(expectedMainToTempForTermination, dataSplitRanges.get(1)), operations.get(1).ingestSql().get(4));
        Assertions.assertEquals(expectedUpdateMainForTermination, operations.get(1).ingestSql().get(5));
        Assertions.assertEquals(expectedTempToMainForTermination, operations.get(1).ingestSql().get(6));
        Assertions.assertEquals(getExpectedCleanupSql(tempName, "legend_persistence_temp"), operations.get(1).ingestSql().get(7));
        Assertions.assertEquals(getExpectedCleanupSql(tempWithDeleteIndicatorName, "legend_persistence_tempWithDeleteIndicator"), operations.get(1).ingestSql().get(8));
        Assertions.assertEquals(getExpectedMetadataTableIngestQuery(), operations.get(0).metadataIngestSql().get(0));
        Assertions.assertEquals(2, operations.size());

        Assertions.assertEquals(BigQueryTestArtifacts.getDropTempTableQuery(tempName), operations.get(0).postCleanupSql().get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.getDropTempTableQuery(tempWithDeleteIndicatorName), operations.get(0).postCleanupSql().get(1));
