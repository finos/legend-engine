...
public class UnitemporalDeltaBatchIdDateTimeBasedTest extends UnitmemporalDeltaBatchIdDateTimeBasedTestCases
{
    ...
    public void verifyUnitemporalDeltaNoDeleteIndFilterDupsAllVersionWithoutPerform(List<GeneratorResult> operations, List<DataSplitRange> dataSplitRanges)
    {
        ...
        Assertions.assertEquals(BigQueryTestArtifacts.expectedMainTableCreateQuery, operations.get(0).preActionsSql().get(0));
    ...
    public void verifyUnitemporalDeltaWithDeleteIndMultiValuesNoDedupNoVersion(GeneratorResult operations)
    {
        ...
        String expectedUpsertQuery = "INSERT INTO `mydb`.`main` " +
                ...
                "(SELECT stage.`id`,stage.`name`,stage.`amount`,stage.`biz_date`,stage.`digest`," +
                "(SELECT COALESCE(MAX(batch_metadata.`table_batch_id`),0)+1 FROM batch_metadata as batch_metadata WHERE UPPER(batch_metadata.`table_name`) = 'MAIN')," +
                "999999999,PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','2000-01-01 00:00:00.000000'),PARSE_DATETIME('%Y-%m-%d %H:%M:%E6S','9999-12-31 23:59:59') FROM `mydb`.`staging_temp_staging_lp_yosulf` as stage " +
                "WHERE (NOT (EXISTS (SELECT * FROM `mydb`.`main` as sink " +
        ...
        Assertions.assertEquals(expectedUpsertQuery, milestoningSql.get(1));
        Assertions.assertEquals(getExpectedMetadataTableIngestQuery(), metadataIngestSql.get(0));
        Assertions.assertEquals(BigQueryTestArtifacts.expectedTempStagingCleanupQuery, dedupAndVersioningSql.get(0));
        Assertions.assertEquals(expectedInsertIntoBaseTempStagingWithFilterDuplicates, dedupAndVersioningSql.get(1));
        Assertions.assertEquals(BigQueryTestArtifacts.maxDupsErrorCheckSql, dedupAndVersionErrorSqlTypeStringMap.get(MAX_DUPLICATES));
        Assertions.assertEquals(BigQueryTestArtifacts.dupRowsSql, dedupAndVersionErrorSqlTypeStringMap.get(DUPLICATE_ROWS));
        Assertions.assertEquals(BigQueryTestArtifacts.maxPkDupsErrorCheckSql, dedupAndVersionErrorSqlTypeStringMap.get(MAX_PK_DUPLICATES));
        Assertions.assertEquals(BigQueryTestArtifacts.dupPkRowsSql, dedupAndVersionErrorSqlTypeStringMap.get(PK_DUPLICATE_ROWS));
