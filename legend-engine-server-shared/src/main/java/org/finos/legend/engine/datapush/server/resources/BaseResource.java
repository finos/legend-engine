...
import org.finos.legend.engine.datapush.server.monitoring.DataPushServerMetricsHandler;
...
public abstract class BaseResource
{
    protected <T> T execute(String descriptionForLogging, String metricName, Supplier<T> supplier)
    {
        ...
        DataPushServerMetricsHandler.operationStart();
        ...
        try
        {
            ...
            DataPushServerMetricsHandler.operationComplete(startTime, endTime, metricName);
            if (isInfoLogging)
            {
        ...
        catch (DataPushServerException e)
        {
            long endTime = System.nanoTime();
            Response.Status status = e.getStatus();
            if ((status != null) && (status.getFamily() == Response.Status.Family.REDIRECTION))
            {
                DataPushServerMetricsHandler.operationRedirect(startTime, endTime, metricName);
                if (isInfoLogging)
                {
                    long duration = endTime - startTime;
            ...
            {
                DataPushServerMetricsHandler.operationError(startTime, endTime, metricName);
