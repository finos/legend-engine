...
public class ModelToSchemaConfigDeserializer extends JsonDeserializer
{

    private static final ObjectMapper objectMapper = ObjectMapperFactory.getNewStandardObjectMapperWithPureProtocolExtensionSupports();
    private static final Map<String, ExternalFormatExtension<?>> extensions = ExternalFormatExtensionLoader.extensions();
    ...
    public Object deserialize(JsonParser jsonParser, DeserializationContext deserializationContext) throws IOException, JsonProcessingException
    {
        ...
        Class<?> configType = Arrays.stream(extensions.get(format).getClass().getGenericInterfaces()).filter(ParameterizedType.class::isInstance).map(ParameterizedType.class::cast)
            .filter(pt -> pt.getRawType().equals(ExternalFormatSchemaGenerationExtension.class)).findFirst().map(pt -> pt.getActualTypeArguments()[1]).map(Class.class::cast)
