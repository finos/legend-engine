...
public class TestPlanExecutionWithAuthenticationFlow extends AbstractTestPlanExecution
{
    ...
    public void testPlanExecutionWithLocalH2() throws Exception
    {
        ...
        RelationalResult result = (RelationalResult) executionPlan.rootExecutionNode.accept(new ExecutionNodeExecutor(IdentityFactoryProvider.getInstance().getAnonymousIdentity(), new ExecutionState(Maps.mutable.empty(), Lists.mutable.withAll(executionPlan.templateFunctions), Lists.mutable.with(new RelationalStoreExecutionState(new RelationalStoreState(port))))));
