...
public class PureIDELight extends PureIDEServer
{
    ...
    {
        new PureIDELight().run(args.length == 0 ? new String[]{"server", "legend-engine-xt-relationalStore-databricks-pure/src/test/resources/ideLightConfig.json"} : args);
    }

    @Override
    protected MutableList<RepositoryCodeStorage> buildRepositories(SourceLocationConfiguration sourceLocationConfiguration)
    {
        String databricksRepo = "legend-engine-xt-relationalStore-databricks-pure/src/main/resources/core_relational_databricks";
        return Lists.mutable.<RepositoryCodeStorage>empty()
                .with(new MutableFSCodeStorage(GenericCodeRepository.build(Paths.get(databricksRepo + ".definition.json")), Paths.get(databricksRepo)));
    }
