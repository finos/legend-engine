...
public final class ConcurrentExecutionNodeExecutorPool
{
    ...
    public static ConcurrentExecutionNodeExecutorPool getOrInitializeExecutorPool(int poolSize)
    {
        return INSTANCE.updateAndGet(existing ->
        {
            if (existing == null)
            {
                return new ConcurrentExecutionNodeExecutorPool(poolSize);
            }
            else
            {
                return existing;
    ...
    public List<Result> execute(final List<ExecutionNode> nodes, final MutableList<CommonProfile> profiles, final ExecutionState executionState)
    {
                ...
                TracedSupplier.reActivateSpan(() ->
                        {
                            try (Scope scope = GlobalTracer.get().buildSpan(String.format("Execution for child - %d", nodes.indexOf(node))).startActive(true))
                            {
                                ...
                                ExecutionState executionStateForThread = executionState.deepCopy();
                                Result result = node.accept(new ExecutionNodeExecutor(profiles, executionStateForThread));
