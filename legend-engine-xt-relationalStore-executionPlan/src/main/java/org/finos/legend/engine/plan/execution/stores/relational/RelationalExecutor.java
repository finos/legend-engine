...
public class RelationalExecutor
{
    ...
    public RelationalExecutor(TemporaryTestDbConfiguration temporarytestdb, RelationalExecutionConfiguration relationalExecutionConfiguration, RequestIdGenerator requestIdGenerator)
    {
    ...
    {
        String databaseTimeZone = ((RelationalStoreExecutionState) executionState.getStoreExecutionState(StoreType.Relational)).inScopeDatabaseConnection.timeZone == null ? DEFAULT_DB_TIME_ZONE : ((RelationalStoreExecutionState) executionState.getStoreExecutionState(StoreType.Relational)).inScopeDatabaseConnection.timeZone;
        String databaseType = ((RelationalStoreExecutionState) executionState.getStoreExecutionState(StoreType.Relational)).inScopeDatabaseConnection.type.name();
        List<String> tempTableList = FastList.newList();

        node.sqlCommands.forEach(sqlQuery ->
        {
            Connection connectionManagerConnection = ((RelationalStoreExecutionState) executionState.getStoreExecutionState(StoreType.Relational)).inScopeConnection;
