...
public class RelationalResult extends StreamingResult implements IRelationalResult
{
     ...
     */
    @Deprecated
    private MutableList<SetImplTransformers> setTransformersWithoutDateTransformations = Lists.mutable.empty();
    ...
    private void buildTransformersAndBuilder(ExecutionNode node, DatabaseConnection databaseConnection) throws SQLException
    {
        ...
        if (ExecutionNodeTDSResultHelper.isResultTDS(node))
        {
            ...
            setTransformersWithoutDateTransformations.add(new SetImplTransformers(transformerInputs, false));
        ...
        else if (ExecutionNodeRelationalResultHelper.isRelationResult(node))
        {
            ...
            SetImplTransformers setImplWithoutDateTransformations = new SetImplTransformers();
    ...
    {
        return this.setTransformersWithoutDateTransformations.size() == 1 ? this.setTransformersWithoutDateTransformations.get(0).transformers : this.setTransformersWithoutDateTransformations.get(this.resultSet.getInt("u_type")).transformers;
    }
    ...
    public Serializer getSerializer(SerializationFormat format)
    {
        switch (format)
        {
            ...
            case CSV_LEGACY:
                return new RelationalResultToCSVSerializerWithoutISODateTransformations(this, true);
            case DEFAULT:
