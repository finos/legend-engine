...
public class TestFreemarker
{
    ...
    public void testSimplePlaceHolder() throws Exception
    {
        ...
        Assert.assertEquals("Select \"root\".date as \"date\" from testTable as \"root\" where \"date\" >'2018-10-15'", result);
    }
    ...
    public void testSimplePlaceHolderObjectproperty() throws Exception
    {
        ...
        String result = RelationalExecutor.process(testQuery, rootVars, functionTemplates());
        Assert.assertEquals("Select \"root\".date as \"date\" from testTable as \"root\" where \"date\" >'2018-10-15'", result);
    }
    ...
    {
        String query = "final collection : ${renderCollection(testCollection ',' '' '' 'null')}";
    ...
    {
        String query = "final collection : ${renderCollection(testCollection \",\" \"convert(DATETIME, '\" \"', 121)\" \"null\")}";
        List collection = new ArrayList();
        Map rootMap = new HashMap();
        rootMap.put("testCollection", collection);
        String resultWithEmptyCollection = RelationalExecutor.process(query, rootMap, functionTemplates());
    ...
    {
        return String.join("\n", new String[] {collectionTemplate(), collectionSizeTemplate(), escapeSqlTemplate(), collectionTemplateWithTz(), enumMap_test_Map_CaseTypeMapping(), enumMap_test_Map_CountryMapping(), equalEnumOperationSelector()});
    }
//corresponds to function templates coming with plan

    public static String collectionTemplateWithTz()
    {
    ...
    {
        return core_relational_relational_relationalMappingExecution.Root_meta_relational_mapping_collectionRenderFunction__String_1_(null);
    }
    ...
    public static String escapeSqlTemplate()
    {
        return core_relational_relational_relationalMappingExecution.Root_meta_relational_mapping_escapeSqlFunction__String_1_(null);
    }

    public static String collectionSizeTemplate()
    {
        return core_relational_relational_relationalMappingExecution.Root_meta_relational_mapping_collectionSizeFunction__String_1_(null);
    }

    @Test
    public void escapedString() throws Exception
    {
        String testQuery = "View of string == '${escapeSql(id)}'";
        Map<String, Result> vars = new HashMap<>();
        vars.put("id", new ConstantResult("O'Connor"));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of string == 'O''Connor'", result);
    }

    @Test
    public void escapedCollection() throws Exception
    {
        String testQuery = "View of collection in (${renderCollection(id \",\" \"'\" \"'\" \"null\" true)})";
        Map<String, Result> vars = new HashMap<>();
        vars.put("id", new ConstantResult(Lists.mutable.with("O'Connor", "O'Malley")));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of collection in ('O''Connor','O''Malley')", result);
    }
    ...
    public void unknownVariable() throws Exception
    {
        String testQuery = "View of string == '${escapeSql(id)}'";
        Map<String, Result> vars = new HashMap<>();
        vars.put("id", new ConstantResult("This is ${bad}"));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of string == 'This is ${bad}'", result);
    }

    @Test
    public void recursiveRenderingNoFunctionNoBrackets() throws Exception
    {
        String testQuery = "View of string == '${id}'";
        Map<String, Result> vars = new HashMap<>();
        vars.put("id", new ConstantResult("Infinite loop ${id}"));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of string == 'Infinite loop Infinite loop ${id}'", result);
    }

    @Test
    public void recursiveRendering() throws Exception
    {
        String testQuery = "View of string == '${escapeSql(id)}'";
        Map<String, Result> vars = new HashMap<>();
        vars.put("id", new ConstantResult("Infinite loop ${id}"));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of string == 'Infinite loop ${id}'", result);
    }

    @Test
    public void nestedVariable() throws Exception
    {
        String testQuery = "View of a (<@a?interpret/>)";
        Map<String, Result> vars = new HashMap<>();
        vars.put("a", new ConstantResult("${b}"));
        vars.put("b", new ConstantResult("c"));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state);
        Assert.assertEquals("View of a (c)", result);
    }

    @Test
    public void nestedVariableDate() throws Exception
    {
        //String testQuery = "View of a (<@a?interpret/>)";
        String testQuery = "View of a (${a})";
        Map<String, Result> vars = new HashMap<>();
        vars.put("a", new ConstantResult("${renderCollectionWithTz(birthTime \"[US/Arizona]\" \",\" \"'\" \"'\" \"null\")}"));
        vars.put("birthTime", new ConstantResult(Lists.mutable.with(EngineDate.fromDateString("2020-12-13 03:00:00"))));
        ExecutionState state = new ExecutionState(vars, Lists.mutable.with(functionTemplates()), Lists.mutable.empty());
        String result = FreeMarkerExecutor.process(testQuery, state, "");
        Assert.assertEquals("View of a ('2020-12-12 20:00:00')", result);
    }
    ...
    public void testFreeMarkerInstanceOfMethod()
    {
        ...
        String testQueryBad = "testQueryBad - Input is a stream - ${instanceOf(input, \"Bad\")?c}";
        vars.put("input", new ConstantResult("string"));
        ExecutionState state3 = new ExecutionState(vars, Lists.mutable.empty(), Lists.mutable.empty());
        String result3 = FreeMarkerExecutor.process(testQueryBad, state3);
        Assert.assertEquals("testQueryBad - Input is a stream - false", result3);
    }
}
