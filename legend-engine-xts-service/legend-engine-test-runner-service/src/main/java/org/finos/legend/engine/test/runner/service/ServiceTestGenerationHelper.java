...
public class ServiceTestGenerationHelper
{
    ...
    private static boolean hasMultipleConnection(Runtime runtime, PureModel pureModel)
    {
        ...
        if (runtime instanceof RuntimePointer)
        {
            RichIterable<? extends Root_meta_core_runtime_ConnectionStore> connections = pureModel.getRuntime(((RuntimePointer) runtime).runtime)._connectionStores();
    ...
    private static boolean doesNotContainRelational(Runtime runtime, PureModel pureModel)
    {
        ...
        if (runtime instanceof RuntimePointer)
        {
            List<? extends Root_meta_core_runtime_ConnectionStore> connections = pureModel.getRuntime(((RuntimePointer) runtime).runtime)._connectionStores().toList();
            return (connections != null) && (connections.size() == 1) && !(connections.get(0)._connection() instanceof Root_meta_external_store_relational_runtime_DatabaseConnection);
        }
        throw new UnsupportedOperationException("Unsupported runtime type: " + runtime.getClass().getName());
    }
    // Relational
