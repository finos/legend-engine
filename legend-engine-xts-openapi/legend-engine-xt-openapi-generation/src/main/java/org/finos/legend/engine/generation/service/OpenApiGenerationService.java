...
public class OpenApiGenerationService
{
    ...
    private Response exec(PureModelContext context, Function0<PureModel> pureModelFunc, boolean interactive, MutableList<CommonProfile> profiles)
    {
        try
        {
            ...
            String result = core_external_format_openapi_transformation_fromPure_pureToOpenApi.Root_meta_external_format_openapi_transformation_fromPure_serviceToOpenApi_Service_1__Server_1__String_1_(pureService, new Root_meta_external_format_openapi_metamodel_Server_Impl("${HOST}"), pureModel.getExecutionSupport());
            LOGGER.debug("OpenAPI generation from pureModelContext completed in {} ms", System.currentTimeMillis() - start);
