...
public class AbstractDriverTest
{
    ...
    protected <T> List<IChecked<T>> deserialize(Class<T> clazz, FlatData flatData, InputStream data)
    {
        ...
        flatData.sections.forEach(s -> deserializer.withSectionDetails(s.name, clazz, true));
    ...
    public static class Deserializer<T>
    {
        ...
        public List<IChecked<T>> deserialize()
        {
            ...
            flatData.sections.forEach(s -> builder.withToObjectFactoryFactory(s.name, x -> reflectiveToObject(s.name, x)));
                    ...
                    try
                    {
                        ...
                        for (Field field : clazz.getFields())
                        {
                            ...
                            for (Predicate<String> matcher : matchers)
                            {
                                if (fdField == null)
                                {
                                    fdField = type.fields.stream().filter(f -> matcher.test(f.label)).findFirst().orElse(null);
                                }
                            }
