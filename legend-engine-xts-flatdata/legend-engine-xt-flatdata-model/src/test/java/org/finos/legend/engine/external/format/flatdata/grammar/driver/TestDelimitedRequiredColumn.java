...
public class TestDelimitedRequiredColumn extends AbstractDriverTest
{
    ...
    public void byDefaultOptionalColumnsCanMeMissing()
    {
        ...
        assertSource(1, 2, "John", Arrays.asList(AbstractDriverTest.rValue("NAME", "John")), records.get(0));
        Person p1 = records.get(0).getValue();
        Assert.assertEquals("John", p1.name);
        Assert.assertNull(p1.weight);

        assertSource(2, 3, "Will", Arrays.asList(AbstractDriverTest.rValue("NAME", "Will")), records.get(1));
    ...
    public void whenPresentRequiredFieldsCanBeOptional()
    {
        ...
        records.forEach(this::assertNoDefects);
        assertSource(1, 2, "John,75", Arrays.asList(AbstractDriverTest.rValue("NAME", "John"), AbstractDriverTest.rValue("WEIGHT", "75")), records.get(0));
        Person p1 = records.get(0).getValue();
        Assert.assertEquals("John", p1.name);
    ...
    public void byDefaultUnmodelledColumnsCanBePresent()
    {
        ...
        records.forEach(this::assertNoDefects);
        assertSource(1, 2, "John,Reading", Arrays.asList(AbstractDriverTest.rValue("NAME", "John"), AbstractDriverTest.rValue("HOBBY", "Reading")), records.get(0));
        Assert.assertEquals("John", records.get(0).getValue().name);
        assertSource(2, 3, "Will,Singing", Arrays.asList(AbstractDriverTest.rValue("NAME", "Will"), AbstractDriverTest.rValue("HOBBY", "Singing")), records.get(1));
