...
import org.finos.legend.engine.external.format.flatdata.driver.core.connection.ObjectCursor;
...
public abstract class DelimitedWriteDriver<T> implements FlatDataWriteDriver<T>
{
    ...
    DelimitedWriteDriver(DelimitedDriverHelper helper)
    {
        ...
        this.helper = helper;
        this.recordType = Objects.requireNonNull(helper.section.recordType);
    ...
    void writeDataLines(PrintWriter writer)
    {
        ...
        final List<FlatDataRecordField> fields = recordType.fields;
        ...
        while (!cursor.isEndOfData())
        {
            ...
            for (int i = 0; i < values.length; i++)
            {
                ...
                if (values[i] == null && !type.optional)
                {
                    throw new IllegalStateException("No value found for mandatory field '" + field.label + "'");
                }
            }
