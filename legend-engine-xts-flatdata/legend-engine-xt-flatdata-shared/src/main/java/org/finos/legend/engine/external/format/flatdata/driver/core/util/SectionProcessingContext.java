...
import org.finos.legend.engine.external.format.flatdata.driver.core.variables.VariablesProcessingContext;
...
public class SectionProcessingContext extends VariablesProcessingContext
{
    ...
    {
        return (ParsedFlatDataToObject<? extends T>) Objects.requireNonNull(toObjectFactoryFactory, "No factory for section '" + section.name + "'").apply(recordType);
    }

    @Override
    public <T> ObjectToParsedFlatData<? extends T> createFromObjectFactory(FlatDataRecordType recordType)
    {
        return (ObjectToParsedFlatData<? extends T>) Objects.requireNonNull(fromObjectFactoryFactory, "No factory for section '" + section.name + "'").apply(recordType);
    }
}
