...
package org.finos.legend.engine.external.format.flatdata.driver.core.data;

import org.eclipse.collections.impl.factory.Lists;
import org.finos.legend.engine.external.format.flatdata.driver.core.fieldHandler.FieldHandler;
import org.finos.legend.engine.external.format.flatdata.driver.core.fieldHandler.FieldHandlerRecordField;
...
public class LazyParsedFlatData implements ParsedFlatData
{
    ...
    {
        String suffix = "'" + handler.getField().label + "' with value: " + handler.rawValue(rawFlatData) + ", error: ParseException " + errorMessage;
        IDefect defect = handler.getField().type.optional
    ...
    {
        IDefect defect = BasicDefect.newInvalidInputCriticalDefect("Failed to read '" + handler.getField().label + "' not present in the source", definingPath);
        defects.add(defect);
        states[handler.getFieldIndex()] = FieldState.INVALID;
    }
    ...
    private FieldHandler fieldHandler(FlatDataRecordField field)
    {
        return (field instanceof FieldHandlerRecordField)
                ...
                : fieldHandlers.stream().filter(f -> f.getField().label.equals(field.label)).findFirst().orElseThrow(() -> new IllegalArgumentException("No field handler for " + field.label));
    }
}
