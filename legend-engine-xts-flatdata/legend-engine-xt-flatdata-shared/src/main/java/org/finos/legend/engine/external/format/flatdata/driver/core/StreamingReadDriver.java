...
import org.finos.legend.engine.external.format.flatdata.driver.core.variables.IntegerVariable;
...
public abstract class StreamingReadDriver<T> implements FlatDataReadDriver<T>
{
    ...
    {
        this.helper = helper;
        this.commonDataHandler = helper.section.recordType == null ? null : new CommonDataHandler(helper.section, helper.context);
    ...
    {
        if (FlatDataUtils.getBoolean(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.UNTIL_EOF))
        {
            return true;
        }
        else if (FlatDataUtils.getBoolean(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.DEFAULT))
        {
            return false;
        }
        else
        {
            ...
            try
            {
                long untilLines = FlatDataUtils.getInteger(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.FOR_NUMBER_OF_LINES).orElseThrow(() -> new IllegalStateException("Expected number of lines"));
    ...
    private class RawLines
    {
        ...
        private void readLines()
        {
            try
            {
                ...
                else if (FlatDataUtils.getString(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.UNTIL_LINE_EQUALS).isPresent())
                {
                    String untilText = FlatDataUtils.getString(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.UNTIL_LINE_EQUALS).get();
                    sectionHasConsumedAllItsRawLines = () -> lastLine != null && untilText.equals(lastLine.getText());
                }
                else if (FlatDataUtils.getInteger(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.FOR_NUMBER_OF_LINES).isPresent())
                {
                    long untilLines = FlatDataUtils.getInteger(helper.section.sectionProperties, StreamingDriverHelper.SCOPE, StreamingDriverHelper.FOR_NUMBER_OF_LINES).get();
