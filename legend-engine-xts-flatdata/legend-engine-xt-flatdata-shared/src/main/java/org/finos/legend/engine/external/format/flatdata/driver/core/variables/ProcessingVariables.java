...
package org.finos.legend.engine.external.format.flatdata.driver.core.variables;
...
public class ProcessingVariables
{
    ...
    public ProcessingVariables(FlatData schema)
    {
        ...
        for (FlatDataSection section : schema.sections)
        {
            FlatDataDriverDescription description = descriptions.stream().filter(d -> d.getId().equals(section.driverId)).findFirst().orElseThrow(() -> new RuntimeException("No driver for: '" + section.driverId + "'"));
