...
public class TestDataGeneration
{
    ...
    @POST
    @Path("relationalCSVTable")
    @ApiOperation(value = "Studio WIP: Generate RelationalCSVTable")
    @Consumes({MediaType.APPLICATION_JSON, APPLICATION_ZLIB})
    public Response generateRelationalCSVTable(TestDataGenerationInput input, @ApiParam(hidden = true) @Pac4JProfileManager ProfileManager<CommonProfile> profileManager)
    {
        ...
        try
        {
            RelationalCSVTableGenerationService relationalCSVTableGenerationService = new RelationalCSVTableGenerationService();
            List<RelationalCSVTable> tables = relationalCSVTableGenerationService.generateRelationalCSVTable(mapping, pureModel);
            return ManageConstantResult.manageResult(profiles, tables, objectMapper);
        }
        catch (Exception e)
        {
