...
import org.finos.legend.engine.protocol.pure.v1.model.data.PureCollectionData;
...
public class PureCollectionDataParseTreeWalker
{
    ...
    public PureCollectionDataParseTreeWalker(ParseTreeWalkerSourceInformation walkerSourceInformation, SourceInformation sourceInformation)
    {
        this.walkerSourceInformation = walkerSourceInformation;
        this.sourceInformation = sourceInformation;
    }

    public PureCollectionData visit(PureCollectionDataParserGrammar.DefinitionContext ctx)
    {
        PureCollectionDataParserGrammar.DataContext dataContext = PureGrammarParserUtility.validateAndExtractRequiredField(ctx.data(), "data", sourceInformation);

        PureCollectionData result = new PureCollectionData();
        result._type = "PureCollection";
        result.sourceInformation = sourceInformation;
        result.data = collection(dataContext.instance().stream().map(this::visitInstance).collect(Collectors.toList()));
        return result;
    }
    ...
    private KeyExpression visitPropertyAssignment(PureCollectionDataParserGrammar.InstancePropertyAssignmentContext ctx)
    {
    ...
    private ValueSpecification visitAtomicRightSide(PureCollectionDataParserGrammar.InstanceAtomicRightSideContext ctx)
    {
