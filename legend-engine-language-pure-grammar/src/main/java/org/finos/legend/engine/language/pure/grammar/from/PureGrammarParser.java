...
public class PureGrammarParser
{
    ...
    public PureModelContextData parse(String code, DEPRECATED_PureGrammarParserLibrary parserLibrary, List<PureGrammarParserExtension> extensions)
    {
        ...
        // create blank PURE model context data
        PureModelContextData pureModelContextData = new PureModelContextData();
        pureModelContextData.stores = new ArrayList<>();
        pureModelContextData.mappings = new ArrayList<>();
        pureModelContextData.services = new ArrayList<>();
        pureModelContextData.stores = new ArrayList<>();
        pureModelContextData.diagrams = new ArrayList<>();
        pureModelContextData.flattenSpecifications = new ArrayList<>();
        pureModelContextData.texts = new ArrayList<>();
        pureModelContextData.runtimes = new ArrayList<>();
        pureModelContextData.connections = new ArrayList<>();
        pureModelContextData.fileGenerations = new ArrayList<>();
        pureModelContextData.cacheables = new ArrayList<>();
        pureModelContextData.caches = new ArrayList<>();
        pureModelContextData.pipelines = new ArrayList<>();
        pureModelContextData.generationSpecifications = new ArrayList<>();
        pureModelContextData.serializableModelSpecifications = new ArrayList<>();
        ...
        sectionIndex.sections = ListIterate.collect(parser.definition().section(), sectionCtx -> this.visitSection(sectionCtx, parserLibrary, extensions, walkerSourceInformation, parserContext, pureModelContextData));
        pureModelContextData.sectionIndices.add(sectionIndex);
        return pureModelContextData;
    }

    private Section visitSection(CodeParserGrammar.SectionContext ctx, DEPRECATED_PureGrammarParserLibrary parserLibrary, List<PureGrammarParserExtension> extensions, ParseTreeWalkerSourceInformation walkerSourceInformation, PureGrammarParserContext parserContext, PureModelContextData pureModelContextData)
    {
        List<Function3<SectionSourceCode, PureModelContextData, PureGrammarParserContext, Section>> extraSectionParsers = ListIterate.flatCollect(extensions, PureGrammarParserExtension::getExtraSectionParsers);
        ...
        if (ctx.sectionContent() != null)
        {
            try
            {
                ...
                Section section = extraSectionParsers.stream().map(sectionParser -> sectionParser.value(codeSection, pureModelContextData, parserContext)).filter(Objects::nonNull).findFirst().orElse(null);
                if (section == null)
                {
                    ...
                    section = parser.parse(parser.getParserInfo(codeSection.code, codeSection.sourceInformation, codeSection.walkerSourceInformation), pureModelContextData, parserContext);
                }
