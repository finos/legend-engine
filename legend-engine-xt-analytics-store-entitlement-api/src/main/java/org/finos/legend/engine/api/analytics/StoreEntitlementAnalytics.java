...
public class StoreEntitlementAnalytics
{
    ...
    public Response generateDatasetSpecifications(StoreEntitlementAnalyticsInput input, @ApiParam(hidden = true) @Pac4JProfileManager ProfileManager<CommonProfile> pm)
    {
        ...
        {
            List<DatasetSpecification> datasets = LazyIterate.flatCollect(extensions, extension -> extension.generateDatasetSpecifications(input.query, input.runtimePath, runtime, input.mappingPath, mapping, input.model, pureModel, modelManager, profiles)).toList();
