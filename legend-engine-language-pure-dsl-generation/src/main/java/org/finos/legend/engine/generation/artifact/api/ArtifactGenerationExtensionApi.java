...
import static org.finos.legend.engine.shared.core.operational.logs.LoggingEventType.GENERATE_ARTIFACT_EXTENSIONS_ERROR;
import static org.finos.legend.engine.shared.core.operational.logs.LoggingEventType.GENERATE_ARTIFACT_EXTENSIONS_START;
import static org.finos.legend.engine.shared.core.operational.logs.LoggingEventType.GENERATE_ARTIFACT_EXTENSIONS_STOP;
...
public class ArtifactGenerationExtensionApi
{
    ...
    public Response generate(ArtifactGenerationExtensionInput artifactGenerationExtensionInput, @ApiParam(hidden = true) @Pac4JProfileManager ProfileManager<CommonProfile> pm)
    {
        ...
        {
            LOGGER.info(new LogInfo(profiles, GENERATE_ARTIFACT_EXTENSIONS_START).toString());
            ...
            LOGGER.info(new LogInfo(profiles, GENERATE_ARTIFACT_EXTENSIONS_STOP).toString());
            return ManageConstantResult.manageResult(profiles, artifactGenerationExtensionOutput, objectMapper);

        }
        catch (Exception ex)
        {
            return ExceptionTool.exceptionManager(ex, GENERATE_ARTIFACT_EXTENSIONS_ERROR, profiles);
