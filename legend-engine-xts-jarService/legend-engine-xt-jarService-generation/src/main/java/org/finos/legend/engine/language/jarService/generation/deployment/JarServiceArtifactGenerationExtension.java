...
public class JarServiceArtifactGenerationExtension implements ArtifactGenerationExtension
{
    private static  final ObjectMapper mapper = ObjectMapperFactory.getNewStandardObjectMapperWithPureProtocolExtensionSupports();

    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(JarServiceArtifactGenerationExtension.class);

    public final String ROOT_PATH = "Jar-Service-Artifact-Generation";

    private static final String FILE_NAME = "jarServiceArtifact.json";
    private static final String LINEAGE_FILE_NAME = "jarServiceLineageArtifact.json";
    ...
    {
       return element instanceof Root_meta_external_function_activator_jarService_JarService;
    }
    ...
    public List<Artifact> generate(PackageableElement element, PureModel pureModel, PureModelContextData data, String clientVersion)
    {
        ...
        try
        {
            LOGGER.info("Generating jarService deploy artifact for " + element.getName());
            Root_meta_external_function_activator_jarService_JarService activator = (Root_meta_external_function_activator_jarService_JarService) element;
            ...
            GenerationInfoData info = JarServiceArtifactGenerator.renderArtifact(pureModel, activator, data, clientVersion, routerExtensions);
            PureModelContextData serviceData = JarServiceArtifactGenerator.fetchJarService(activator, data, pureModel);
            result.add(new Artifact(mapper.writeValueAsString(new JarServiceArtifact(generatePointerForActivator(serviceData, data), ((Root_meta_external_function_activator_DeploymentOwnership)activator._ownership())._id(), FunctionActivatorGenerator.generateActions(activator, pureModel), (AlloySDLC) data.origin.sdlcInfo)), FILE_NAME, "json"));
            LOGGER.info("Generated artifacts for " + element.getName());

        }
        catch (Exception e)
        {
            LOGGER.error("Error generating jarService artifacts ", e);
        }
        return result;
    }
    ...
    public PureModelContextPointer generatePointerForActivator(PureModelContextData activator, PureModelContextData originalModel)
    {
        ...
        origin.sdlcInfo.packageableElementPointers = Lists.mutable.with(new PackageableElementPointer(PackageableElementType.SERVICE, activator.getElementsOfType(JarService.class).get(0).getPath()),
    ...
    public String elementName(JarService service)
    {
