...
public class MasteryParseTreeWalker
{
    ...
    private RecordSource visitRecordSource(MasteryParserGrammar.RecordSourceContext ctx)
    {
        ...
        // record Service
        MasteryParserGrammar.RecordServiceContext recordServiceContext  = PureGrammarParserUtility.validateAndExtractRequiredField(ctx.recordService(), "recordService", source.sourceInformation);
        source.recordService = visitRecordService(recordServiceContext);

        // trigger
        MasteryParserGrammar.TriggerContext triggerContext = PureGrammarParserUtility.validateAndExtractRequiredField(ctx.trigger(), "trigger", source.sourceInformation);
    ...
    private RecordService visitRecordService(MasteryParserGrammar.RecordServiceContext ctx)
    {
        ...
        MasteryParserGrammar.AcquisitionProtocolContext acquisitionProtocolContext = PureGrammarParserUtility.validateAndExtractRequiredField(ctx.acquisitionProtocol(), "acquisitionProtocol", sourceInformation);
