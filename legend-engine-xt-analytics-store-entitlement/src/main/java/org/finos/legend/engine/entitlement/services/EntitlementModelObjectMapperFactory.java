...
public class EntitlementModelObjectMapperFactory
{
    ...
    private static <T> T withEntitlementModelExtensions(T mapperOrBuilder, BiConsumer<? super T, ? super Module> addModule, BiConsumer<? super T, ? super NamedType> registerSubTypes)
    {
        ...
        // register sub types
        Map<EntitlementSubtypeInfo<?>, EntitlementServiceExtension> subTypeInfoByExtension = Maps.mutable.empty();
        Map<Class<?>, EntitlementSubtypeInfo<?>> superTypesWithDefaultRegisteredSubtype = Maps.mutable.empty();
        Map<Class<?>, EntitlementSubtypeInfo<?>> registeredSubTypes = Maps.mutable.empty();
        EntitlementServiceExtensionLoader.extensions().forEach(extension ->
                LazyIterate.flatCollect(extension.getExtraEntitlementSubTypeInfoCollectors(), Function0::value).forEach(info ->
                {
                    ...
                    if (info.getDefaultSubType() != null)
                    {
                        ...
                        {
                            EntitlementSubtypeInfo<?> found = superTypesWithDefaultRegisteredSubtype.get(info.getSuperType());
                    ...
                    info.getSubTypes().forEach(subType ->
                    {
                        ...
                        {
                            EntitlementSubtypeInfo<?> found = registeredSubTypes.get(subType.getOne());
