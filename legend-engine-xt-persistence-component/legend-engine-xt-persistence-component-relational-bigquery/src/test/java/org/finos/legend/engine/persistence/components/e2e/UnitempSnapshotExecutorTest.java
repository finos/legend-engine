...
@Disabled
public class UnitempSnapshotExecutorTest extends BigQueryEndToEndTest
{
    ...
    public void testMilestoning() throws IOException, InterruptedException
    {
        ...
        String expectedPath = "src/test/resources/expected/unitemp_snapshot/data_pass1.csv";
        ...
        expectedPath = "src/test/resources/expected/unitemp_snapshot/data_pass2.csv";
        assertFileAndTableDataEquals(schema, expectedPath, tableData);
        ...
        long rowsTerminated = (long) result.statisticByName().get(ROWS_TERMINATED);

        Assertions.assertEquals(3, incomingRecords);
        Assertions.assertEquals(1, rowsInserted);
        Assertions.assertEquals(1, rowsUpdated);
        Assertions.assertEquals(1, rowsTerminated);
    }
}
