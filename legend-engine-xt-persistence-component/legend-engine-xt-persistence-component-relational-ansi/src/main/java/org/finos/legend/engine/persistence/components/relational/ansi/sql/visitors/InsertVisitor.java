...
public class InsertVisitor implements LogicalPlanVisitor<Insert>
{

    @Override
    public VisitorResult visit(PhysicalPlanNode prev, Insert current, VisitorContext context)
    {
        InsertStatement insertStatement = new InsertStatement();
        prev.push(insertStatement);

        List<LogicalPlanNode> logicalPlanNodeList = new ArrayList<>();
        if (current.sourceDataset() != null)
        {
            logicalPlanNodeList.add(current.sourceDataset());
        }
        logicalPlanNodeList.add(current.targetDataset());
        if (current.fields() == null || current.fields().isEmpty())
        {
