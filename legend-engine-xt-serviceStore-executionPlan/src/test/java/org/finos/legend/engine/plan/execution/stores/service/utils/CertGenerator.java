...
public class CertGenerator
{
    ...
    public Certs generateCerts() throws Exception
    {
        ...
        copyFile("/certs", "server.csr.conf", certsWorkDir, false);
        copyFile("/certs", "client.csr.conf", certsWorkDir, false);
        ...
        File clientKeyStoreFile = generatedWorkDir.resolve("clientkeystore.jks").toFile();
        if (!clientKeyStoreFile.exists())
        {
            throw new Exception("Cert setup not successful. File " + clientKeyStoreFile.toPath().toAbsolutePath() + " does not exist");
        }
        certs.clientKeyStorePath = clientKeyStoreFile.toPath().toAbsolutePath().toString();
    ...
    {
        Path source = Paths.get(MTLSNoClientAuthTest.class.getResource(resourceDir + "/" + fileName).toURI());
